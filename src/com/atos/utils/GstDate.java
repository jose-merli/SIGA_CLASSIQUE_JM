...
public class GstDate {
	...
	public Date parseStringToDate(String date, String format, Locale locale)
		...
		try {
			...
			ClsExceptions exc = new ClsExceptions(e, "error en GstDate     :"
					+ e.toString());
	...
	public static String getFormatedDateShort(String lang, String date)
			throws ClsExceptions {
		String dat = "";
		...
		if (date != null && !date.trim().equals("")) {
			try {
				...
				ClsExceptions exc = new ClsExceptions(ex, "THIS DATE " + date
						+ " IS BAD FORMATED");
				exc.setErrorCode("DATEFORMAT");
				throw exc;
			}
		...
		if (odate != null) {
			...
			if (!date.trim().equals("")) {
				try {
					...
					ClsExceptions exc = new ClsExceptions(ex, "THIS DATE "
							+ date + " IS BAD FORMATED");
					exc.setErrorCode("DATEFORMAT");
					throw exc;
				}
	...
	public static String getFormatedDateLong(String lang, String date)
			throws ClsExceptions {
		String dat = "";
		SimpleDateFormat sdf = new SimpleDateFormat(
				ClsConstants.DATE_FORMAT_JAVA);
	...
	public static String getApplicationFormatDate(String lang, String date)
			throws ClsExceptions {
		...
		if (date.length() == 11 && date.substring(2, 3).equals(".")) {
			...
			if (lang.equalsIgnoreCase("EN")) {
				oDate = new GstDate().parseStringToDate(date,
			...
			if (date.length() > 11) {
				...
				if (lang.equalsIgnoreCase("EN")) {
					sdf = new SimpleDateFormat(
							ClsConstants.DATE_FORMAT_LONG_ENGLISH);
				} else {
					sdf = new SimpleDateFormat(
				...
				if (lang.equalsIgnoreCase("EN")) {
					sdf = new SimpleDateFormat(
							ClsConstants.DATE_FORMAT_SHORT_ENGLISH);
				} else {
					sdf = new SimpleDateFormat(
	...
	/*
	 * public String parseDatetoString(Date date, String format, Locale
	 * locale)throws ClsExceptions{ String dat=""; SimpleDateFormat sdf = new
	 * SimpleDateFormat(format,locale); try { sdf.format(date,new
	 * FieldPosition(0)); // Date d = sdf.parse(date, new ParsePosition(0)); }
	 * catch (Exception e) { throw new
	 * ClsExceptions("error en GstDate     :"+e.toString()); } return dat; }
	 */

	// Daniel Campos
	public static String getFechaLenguaje(String lenguaje, String date)
			throws ClsExceptions {
		SimpleDateFormat sdf = new SimpleDateFormat(
				ClsConstants.DATE_FORMAT_JAVA); // Ingles
		Date datFormat = null;
		...
		if (date != null && !date.equals("")) {
			try {
				...
				try {
					...
					if (date.indexOf(":") > 0) {
						ClsExceptions exc = new ClsExceptions(ex,
								"Formato de fecha no reconocido");
						exc.setErrorCode("DATEFORMAT");
						throw exc;
					} else {
						return GstDate.getFechaLenguaje(lenguaje, date
								+ " 00:00:00");
					}
				}
			}
	 ...
	 * Devuelve un String que contiene una condición AND para buscar registros
	 * cuya fecha se encuentra entre las 0h y las 24h del día (dd/mm/yyyy) que
	 * indica <b>fecha</b>.<br>
	 * Ejemplo: fecha = 03/01/2005, return = AND NOMBRECOLUMNA >= 2005/01/03
	 * 00:00:00 AND NOMBRECOLUMNA <= 2005/01/03 23:59:59
	 * 
	 * @param nombreColumna
	 *            Nombre de la columna a filtrar
	 * @param fecha
	 *            Día en formato dd/mm/yyyy
	 * @return String con condición AND
	 * @throws ClsExceptions
	 **/
	public static String dateBetween0and24h(String nombreColumna, String fecha)
			throws ClsExceptions {
		String result = "";
		SimpleDateFormat sdf = new SimpleDateFormat(
				ClsConstants.DATE_FORMAT_JAVA);
		...
		try {
			...
			result += " AND " + nombreColumna + " <= TO_DATE('" + fechaFin
					+ "', '" + ClsConstants.DATE_FORMAT_SQL + "') ";
	...
	public static String dateSuma24hFormatoJava(String fecha)
			throws ClsExceptions {
		String result = "";
		SimpleDateFormat sdf = new SimpleDateFormat(
				ClsConstants.DATE_FORMAT_JAVA);
	...
	public static String dateSumaDiasJava(String fecha, Integer dias)
			throws ClsExceptions {
		String result = "";
		SimpleDateFormat sdf = new SimpleDateFormat(
				ClsConstants.DATE_FORMAT_JAVA);
	...
	public static String dateMasUnDia(String nombreColumna, String fecha)
			throws ClsExceptions {
		String result = "";
		SimpleDateFormat sdf = new SimpleDateFormat(
				ClsConstants.DATE_FORMAT_SQL);
	...
	public static String anyadeHora(String sFecha) {
		try {
			if (sFecha != null)
				if (sFecha.length() == 10) {
					Calendar hoy = GregorianCalendar.getInstance();
					
					String hora = String.valueOf(hoy.get(Calendar.HOUR_OF_DAY));
					if (hora.length() == 1)
						hora = "0" + hora;
					String minuto = String.valueOf(hoy.get(Calendar.MINUTE));
					if (minuto.length() == 1)
						minuto = "0" + minuto;
					String segundo = String.valueOf(hoy.get(Calendar.SECOND));
					if (segundo.length() == 1)
						segundo = "0" + segundo;

					sFecha += " " + hora + ":" + minuto + ":" + segundo;
				}
	...
	public static String getFormatedDateMedium(String lang, Object odate)
			throws ClsExceptions {
		String dat = "";
		String date = "";
		SimpleDateFormat sdf = new SimpleDateFormat(
