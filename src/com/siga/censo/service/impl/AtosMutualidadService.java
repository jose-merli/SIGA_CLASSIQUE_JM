...
public class AtosMutualidadService extends JtaBusinessServiceTemplate 
	...
	/**
	 * Actualiza el estado de una solicitud. Se usa tanto para el plan
	 * profesional como el seguro gratuito, ya que se basa unicamente en el
	 * idSolicitud
	 * 
	 * @param mutualidadForm
	 *            - Requiere tener dentro el mutualidadBean con el
	 *            idSolicitudAceptada relleno
	 * @param usrBean
	 *            - El usuario que hace la solicitud de estado
	 * @throws Exception
	 */
	...
	private Hashtable getDatosPersona(MutualidadForm mutualidadForm) {
		...
		String edadeshijos = mutualidadForm.getEdadHijo1() + "," + mutualidadForm.getEdadHijo2() + "," + mutualidadForm.getEdadHijo3() + "," + mutualidadForm.getEdadHijo4() + "," + mutualidadForm.getEdadHijo5() + "," + mutualidadForm.getEdadHijo6() + "," + mutualidadForm.getEdadHijo7() + "," + mutualidadForm.getEdadHijo8() + "," + mutualidadForm.getEdadHijo9() + "," + mutualidadForm.getEdadHijo10();
	...
	public MutualidadForm setMutualidadForm(MutualidadForm mutualidadForm, UsrBean usrBean) throws Exception {
		...
		if (combosMutualidad.getEstadosCiviles() != null) {
			List<ValueKeyVO> estadosCiviles = (ArrayList<ValueKeyVO>) combosMutualidad.getEstadosCiviles();
			mutualidadForm.setEstadosCiviles(combosMutualidad.getEstadosCiviles());
			Iterator<ValueKeyVO> iter = estadosCiviles.iterator();
			while (iter.hasNext()) {
				ValueKeyVO vk = iter.next();
				if (vk.getKey().equals(mutualidadForm.getIdEstadoCivil())) {
					mutualidadForm.setEstadoCivil(vk.getValue());
					break;
				}
			}
		} else {
			mutualidadForm.setEstadosCiviles(new ArrayList<ValueKeyVO>());
			mutualidadForm.setEstadoCivil("");
		}
	...
	public MutualidadForm getDatosPersonaFicha(MutualidadForm mutualidadForm, UsrBean usr) throws Exception {
		...
		if (v.size() > 0) {
			...
			mutualidadForm.setFechaNacimiento(UtilidadesString.formatoFecha(UtilidadesHash.getString(per, "FECHANACIMIENTO"), ClsConstants.DATE_FORMAT_JAVA, ClsConstants.DATE_FORMAT_SHORT_SPANISH));
