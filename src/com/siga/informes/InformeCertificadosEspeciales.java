...
/**
 * Realiza un informe PDF mediante FOP o Aspose words de las facturas emitidas
 * según plantilla introducida
 */
...
public class InformeCertificadosEspeciales extends MasterReport {
	...
	public Hashtable<String, Object> getDatosParaAsposeWords(Hashtable datosBase) throws ClsExceptions, SIGAException {
		...
		if (iterador != null && iterador.getValues() != null && iterador.getValues().size() > 0) {
	...
	private Iterador getIterador(Hashtable datosBase) throws ClsExceptions, SIGAException {
		Iterador iterador = null;

		String institucion = this.getUsuario().getLocation();
		// Colocamos los registros
		Vector vDatos = getDatosIterables(datosBase);
		...
		if (vDatos != null) {
			...
			for (int i = 0; i < vDatos.size(); i++) {
				...
				if (tipoCert != null && (tipoCert.equals(Certificado.CERT_TIPO_TURNO) || tipoCert.equals(Certificado.CERT_TIPO_PASANTIA) || tipoCert.equals(Certificado.CERT_TIPO_PASANTIA2) || tipoCert.equals(Certificado.CERT_TIPO_DESPACHO1) || tipoCert.equals(Certificado.CERT_TIPO_DESPACHO2))) {
					UtilidadesHash.set(htDatos, "NEXO_AL", UtilidadesHash.getString(haux, "NEXO_AL"));
				}
				if (tipoCert.equals(Certificado.CERT_TIPO_PASANTIA) || tipoCert.equals(Certificado.CERT_TIPO_PASANTIA2)) {
			...
			iterador.put(tipoCert, vDatosFormato);
		}
		return iterador;
	...
	/**
	 * Obtiene los datos fijos del certificaado
	 * 
	 * @param datosBase
	 * @param registro
	 * @return
	...
	public Vector getDatosIterables(Hashtable ht) throws ClsExceptions, SIGAException {
		...
		if (tipoCert != null && (tipoCert.equals(Certificado.CERT_TIPO_CURSO) || tipoCert.equals(Certificado.CERT_TIPO_SEMINARIOS_CURSOS))) {
			sSQL = getSqlCurso(tipoCert);
		} else if (tipoCert != null && tipoCert.equals(Certificado.CERT_TIPO_TURNO)) {
			sSQL = getSqlTurno(tipoCert);
		} else if (tipoCert != null && (tipoCert.equals(Certificado.CERT_TIPO_PASANTIA) || tipoCert.equals(Certificado.CERT_TIPO_PASANTIA2))) {
	...
	public String getSqlTurno(String tipoCert) {
		String sSQL = getSqlCamposGeneral(tipoCert, "") + ",  (select scs_turno.nombre " + " from scs_turno " + " where scs_turno.idinstitucion=it.idinstitucion " + " and scs_turno.idturno=it.idturno) as nombre_turno, " + " it.fechavalidacion as fechaalta_turno, " + " it.fechabaja as fechabaja_turno, " + " DECODE(it.fechabaja,NULL,'',F_SIGA_GETRECURSO_ETIQUETA('messages.certificado.texto.nexoAL',@idioma@)) as NEXO_AL " + getSqlFromGeneral() + ",  scs_inscripcionturno      it " + getSqlWhereGeneral() + " and it.idinstitucion(+)=c.idinstitucion " + " and it.idpersona(+)=c.idpersona  " + " and it.fechavalidacion(+) <=SYSDATE " +
	...
	public String getSqlCurso(String tipoCert) {
		String sSQL = getSqlCamposGeneral(tipoCert, "") + " , f_siga_getrecurso(cv.descripcion, @idioma@) as NOMBRECURSO, " + " cv.fechainicio as FECHAINICIOCURSO," + "cv.fechafin as FECHAFINCURSO" + getSqlFromGeneral() + " , cen_datoscv  cv," + " cen_tiposcv  tipocv" + getSqlWhereGeneral() + " and cv.idpersona(+)=c.idpersona" + " and cv.idinstitucion(+)=c.idinstitucion" + " and cv.idtipocv=tipocv.idtipocv(+)" + " and tipocv.idtipocv=1    " + " order by cv.fechainicio asc";
		return sSQL;

	}
	public String getSqlGeneral() {
		String sSQL = getSqlCamposGeneral("", "")+ getSqlFromGeneral() + getSqlWhereGeneral() ;
		return sSQL;

	}

	public String getSqlPasantia(String tipoCert) {
		String sSQL = getSqlCamposGeneral(tipoCert, "") + ", f_siga_getrecurso(cv.descripcion, @idioma@) as DESCRIPCIONPASANTIA, " + " cv.fechainicio as FECHAINICIOPASANTIAS, " + " cv.fechafin as FECHAFINPASANTIAS, " + " DECODE(cv.fechafin,NULL,'',F_SIGA_GETRECURSO_ETIQUETA('messages.certificado.texto.nexoAL',@idioma@)) as NEXO_AL " + getSqlFromGeneral() + ", cen_datoscv               cv," + " cen_tiposcv               tipocv" + getSqlWhereGeneral() + " and cv.idinstitucion(+)=c.idinstitucion" + " and cv.idpersona(+)=c.idpersona " + " and cv.idtipocv=tipocv.idtipocv(+) " + " and tipocv.idtipocv=3    " + " order by cv.fechainicio asc";
		return sSQL;

	}

	public String getSqlDespacho(String tipoCert) {
		String sSQL = getSqlCamposGeneral(tipoCert, "") + ", ( d.domicilio||' '||d.codigopostal||' '||" + "   decode(d.idpais,191,((select po.nombre" + "                         from cen_poblaciones po" + "                         where po.idpoblacion=d.idpoblacion)" + "          ),d.poblacionextranjera)) as DIRECCION_DESPACHO," + " d.domicilio   AS DOMICILIO_DESPACHO," + " d.codigopostal AS CODIGOPOSTAL_DESPACHO," + " decode(d.idpais,191,((select po.nombre" + "                       from cen_poblaciones po" + "                       where po.idpoblacion=d.idpoblacion)" + "                      ),d.poblacionextranjera)  AS POBLACION_DESPACHO," + " (select f_siga_getrecurso(c.nombre,@idioma@)" + "  from cen_provincias c" + "   where c.idprovincia=d.idprovincia ) AS PROVINCIA_DESPACHO," + " (select f_siga_getrecurso(c.nombre,@idioma@)" + " from cen_pais c" + " where c.idpais=d.idpais  ) AS PAIS_DESPACHO," +

		" d.telefono1 AS TELEFONO_DESPACHO," + " d.movil AS MOVIL_DESPACHO," + " d.fax1 AS FAX_DESPACHO," + " decode ((select 1 " + "          from dual " + "          where nvl(d.fechacarga,D.FECHAMODIFICACION)<D.FECHAMODIFICACION),1,d.fechacarga,D.FECHAMODIFICACION) AS FECHAALTA_DESPACHO," + " D.FECHABAJA AS FECHABAJA_DESPACHO," + " DECODE(D.FECHABAJA,NULL,'',F_SIGA_GETRECURSO_ETIQUETA('messages.certificado.texto.nexoAL',1)) as NEXO_AL" + getSqlFromGeneral() + " , cen_direcciones           d," + " cen_direccion_tipodireccion tipodir" + getSqlWhereGeneral() + " and tipodir.idinstitucion(+)=d.idinstitucion" + "  and tipodir.idpersona(+)=d.idpersona" + "  and tipodir.iddireccion(+)=d.iddireccion" + "  and d.idpersona(+)=c.idpersona" + "  and d.idinstitucion(+)=c.idinstitucion" + "  and tipodir.idtipodireccion=2" + "  ORDER BY FECHAALTA_DESPACHO desc ";
		return sSQL;

	}

	public String getSqlBanco(String tipoCert) {
		String sSQL = getSqlCamposGeneral(tipoCert, "") + " ,decode(cb.ABONOCARGO,'T',F_SIGA_GETRECURSO_ETIQUETA('censo.tipoCuenta.abonoCargo',1),'A',F_SIGA_GETRECURSO_ETIQUETA('censo.tipoCuenta.abono',1),'C',F_SIGA_GETRECURSO_ETIQUETA('censo.tipoCuenta.cargo',1)) as ABONOCARGO_BANCO, " + " cb.IBAN as CUENTA_BANCO, " + " cb.TITULAR as TITULAR_BANCO, " + " cb.FECHABAJA as FECHABAJA_BANCO, " + " cb.CUENTACONTABLE as CUENTACONTABLE_BANCO, " + " (select nombre from cen_bancos where codigo = cb.CBO_CODIGO) as NOMBRE_BANCO, " + " decode(cb.ABONOSJCS,'0',F_SIGA_GETRECURSO_ETIQUETA('general.no',1),F_SIGA_GETRECURSO_ETIQUETA('general.yes',1)) as SJCS_BANCO, " + " decode(cb.IDPERSONA,@idpersona@,F_SIGA_GETRECURSO_ETIQUETA('general.no',1),F_SIGA_GETRECURSO_ETIQUETA('general.yes',1)) as SOCIEDAD_BANCO  " + getSqlFromGeneral() + ",  cen_cuentasbancarias cb " + getSqlWhereGeneral() + " and cb.idinstitucion(+)=c.idinstitucion " + " and cb.idpersona(+)=c.idpersona  " + " and (cb.IDPERSONA = c.idpersona  " + " AND cb.IDINSTITUCION = c.idinstitucion " + "  OR (cb.IDINSTITUCION,cb.IDPERSONA,cb.IDCUENTA) in " + " 		(SELECT CEN_CUENTASBANCARIAS.IDINSTITUCION,CEN_CUENTASBANCARIAS.IDPERSONA,CEN_CUENTASBANCARIAS.IDCUENTA " + " 	       FROM CEN_COMPONENTES " + " 	       JOIN CEN_CUENTASBANCARIAS ON CEN_COMPONENTES.IDINSTITUCION = CEN_CUENTASBANCARIAS.IDINSTITUCION " + " 	                                AND CEN_COMPONENTES.IDPERSONA = CEN_CUENTASBANCARIAS.IDPERSONA " + " 	      WHERE CEN_COMPONENTES.CEN_CLIENTE_IDPERSONA = c.idpersona  " + "           AND CEN_COMPONENTES.FECHABAJA IS NULL " + " 	        AND CEN_COMPONENTES.CEN_CLIENTE_IDINSTITUCION = c.idinstitucion) )" + " ORDER BY CB.TITULAR ";

		return sSQL;
	}

	public String getSqlComponentes(String tipoCert) {
		String sSQL = getSqlCamposGeneral(tipoCert, "") + ",  cp.NUMCOLEGIADO as NUMCOLEG_COMPONENTE, " + " 	N.NIFCIF as NIFCIF_COMPONENTE, " + "   N.NOMBRE AS NOMBRE_COMPONENTE, " + "   N.APELLIDOS1 AS APELLIDO1_COMPONENTE, " + "   N.APELLIDOS2 AS APELLIDO2_COMPONENTE " + getSqlFromGeneral() + ",  CEN_COMPONENTES cp,CEN_PERSONA N  " + getSqlWhereGeneral() + " and cp.CEN_CLIENTE_IDPERSONA = N.IDPERSONA  " + " and cp.IDPERSONA =@idpersona@ " + " and cp.IDINSTITUCION = @idinstitucion@ " + " and (cp.FECHABAJA IS NULL OR cp.FECHABAJA > SYSDATE)" + " AND no.idinstitucion=cp.IDINSTITUCION " + " AND no.idpersona=cp.IDPERSONA " + " ORDER BY p.NIFCIF ";
		return sSQL;

	}

	public String getSqlDirecciones(String tipoCert) {
		String sSQL = getSqlCamposGeneral(tipoCert, "") + ", " + " f_siga_gettiposdireccion(d.IDINSTITUCION,d.IDPERSONA,d.IDDIRECCION, 1) AS TIPO_DIRECCION, " + " d.domicilio   AS DOMICILIO_DIRECCION," + " d.codigopostal AS CODIGOPOSTAL_DIRECCION," + " decode(d.idpais,191,((select po.nombre" + "                       from cen_poblaciones po" + "                       where po.idpoblacion=d.idpoblacion)" + "                      ),d.poblacionextranjera)  AS POBLACION_DIRECCION," + " (select f_siga_getrecurso(c.nombre,@idioma@)" + "  from cen_provincias c" + "   where c.idprovincia=d.idprovincia ) AS PROVINCIA_DIRECCION," + " (select f_siga_getrecurso(c.nombre,@idioma@)" + " from cen_pais c" + " where c.idpais=d.idpais  ) AS PAIS_DIRECCION," +

		" d.telefono1 AS TELEFONO1_DIRECCION," + " d.fax1 AS FAX1_DIRECCION," + " d.movil AS MOVIL_DIRECCION," + " d.correoelectronico AS EMAIL_DIRECCION," + " d.preferente AS PREFERENTE_DIRECCION," + " decode ((select 1 " + "          from dual " + "          where nvl(d.fechacarga,D.FECHAMODIFICACION)<D.FECHAMODIFICACION),1,d.fechacarga,D.FECHAMODIFICACION) AS FECHAALTA_DIRECCION," + " D.FECHABAJA AS FECHABAJA_DIRECCION," + " DECODE(D.FECHABAJA,NULL,'',F_SIGA_GETRECURSO_ETIQUETA('messages.certificado.texto.nexoAL',1)) as NEXO_AL" + getSqlFromGeneral() + " , cen_direcciones           d " + getSqlWhereGeneral() + "  and d.idpersona(+)=c.idpersona" + "  and d.idinstitucion(+)=c.idinstitucion" + "  ORDER BY FECHAALTA_DIRECCION desc ";
		return sSQL;
	...
	public String getSqlCamposGeneral(String tipoCert, String contador) {
		...
		if (!Certificado.CERT_TIPO_DESPACHO1.equals(tipoCert) && !Certificado.CERT_TIPO_DESPACHO2.equals(tipoCert)) {
			sql.append("F_SIGA_GETDIRECCIONDESPACHO(@idinstitucion@, @idpersona@, @idioma@, null) AS DIRECCION_DESPACHO, ");
			sql.append("F_SIGA_GETDIRECCIONDESPACHO(@idinstitucion@, @idpersona@, @idioma@, 1) AS DOMICILIO_DESPACHO, ");
			sql.append("F_SIGA_GETDIRECCIONDESPACHO(@idinstitucion@, @idpersona@, @idioma@, 2) AS CODIGOPOSTAL_DESPACHO, ");
			sql.append("F_SIGA_GETDIRECCIONDESPACHO(@idinstitucion@, @idpersona@, @idioma@, 3) AS POBLACION_DESPACHO, ");
			sql.append("F_SIGA_GETDIRECCIONDESPACHO(@idinstitucion@, @idpersona@, @idioma@, 4) AS PROVINCIA_DESPACHO, ");
			sql.append("F_SIGA_GETDIRECCIONDESPACHO(@idinstitucion@, @idpersona@, @idioma@, 5) AS PAIS_DESPACHO, ");
			sql.append("F_SIGA_GETDIRECCIONDESPACHO(@idinstitucion@, @idpersona@, @idioma@, 6) AS TELEFONO_DESPACHO, ");
			sql.append("F_SIGA_GETDIRECCIONDESPACHO(@idinstitucion@, @idpersona@, @idioma@, 7) AS MOVIL_DESPACHO, ");
			sql.append("F_SIGA_GETDIRECCIONDESPACHO(@idinstitucion@, @idpersona@, @idioma@, 8) AS F_DESPACHO, ");
		}
		sql.append("F_SIGA_GETDIRECCIONCLIENTE(@idinstitucion@, @idpersona@, "+ClsConstants.TIPO_DIRECCION_FACTURACION+", null) AS DIRECCION_FACTURACION, ");
		sql.append("F_SIGA_GETDIRECCIONCLIENTE(@idinstitucion@, @idpersona@, "+ClsConstants.TIPO_DIRECCION_FACTURACION+", 1) AS DOMICILIO_FACTURACION, ");
		sql.append("F_SIGA_GETDIRECCIONCLIENTE(@idinstitucion@, @idpersona@, "+ClsConstants.TIPO_DIRECCION_FACTURACION+", 2) AS CODIGOPOSTAL_FACTURACION, ");
		sql.append("F_SIGA_GETDIRECCIONCLIENTE(@idinstitucion@, @idpersona@, "+ClsConstants.TIPO_DIRECCION_FACTURACION+", 3) AS POBLACION_FACTURACION, ");
		sql.append("F_SIGA_GETDIRECCIONCLIENTE(@idinstitucion@, @idpersona@, "+ClsConstants.TIPO_DIRECCION_FACTURACION+", 4) AS PROVINCIA_FACTURACION, ");
		sql.append("F_SIGA_GETDIRECCIONCLIENTE(@idinstitucion@, @idpersona@, "+ClsConstants.TIPO_DIRECCION_FACTURACION+", 11) AS TELEFONO_FACTURACION, ");
		sql.append("F_SIGA_GETDIRECCIONCLIENTE(@idinstitucion@, @idpersona@, "+ClsConstants.TIPO_DIRECCION_FACTURACION+", 13) AS MOVIL_FACTURACION, ");
		sql.append("F_SIGA_GETDIRECCIONCLIENTE(@idinstitucion@, @idpersona@, "+ClsConstants.TIPO_DIRECCION_FACTURACION+", 14) AS F_FACTURACION, ");
		sql.append("F_SIGA_GETDIRECCIONCLIENTE(@idinstitucion@, @idpersona@, "+ClsConstants.TIPO_DIRECCION_FACTURACION+", 16) AS EMAIL_FACTURACION, ");
	...
	public String getSqlFromGeneral() {
		return " FROM cen_cliente               c," + " cen_persona               p," + " cen_tratamiento           t," + " cer_solicitudcertificados s," + " cen_colegiado             l, " + " cen_nocolegiado          no ";

	}

	public String getSqlWhereGeneral() {
		return " WHERE c.idpersona = @idPersona@" + " AND c.idinstitucion = @idinstitucion@" + " AND s.idsolicitud = @idsolicitud@" + "  AND c.idpersona = s.idpersona_des" + "  AND c.idpersona = p.idpersona" + "  AND c.idtratamiento = t.idtratamiento" + "  AND l.idpersona(+) = c.idpersona" + "  AND l.idinstitucion(+) = c.idinstitucion" + "  AND (c.idinstitucion = s.idinstitucionorigen or" + "      c.idinstitucion = s.idinstitucion)" + "  AND no.idinstitucion(+)=c.idinstitucion " + "  AND no.idpersona(+)=c.idpersona ";
...
class Iterador {
	...
	public void put(String vIteracion, Vector vDatosFormato) {
		setKey(vIteracion);
		setValues(vDatosFormato);
	}
