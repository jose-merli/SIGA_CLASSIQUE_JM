...
//		String numExpediente = (String)ht.get(DE_CE_NUMEXPEDIENTE);
		...
		Integer anyoExpediente = SIGAServicesHelper.getInteger("año del expediente", (String)datosGeneralesEJG.get(DE_CE_ANYOEXPEDIENTE));		
...
//			else{
				...
				ClsLogging.writeFileLog("El EJG NO tiene dictamen que permita no tener documentos. Se añade error "+anyoExpediente+"/"+numExpediente, 3);
...
//			}
	...
	private boolean compruebaDictamenEJG(Hashtable ht) throws ClsExceptions, SIGAException {
		...
		ClsLogging.writeFileLog("Busqueda de dictamen del EJG. El EJG tiene dictamen que permita no tener documentos(1->Sí, 0->No): " , 3);
...
//		fos.close();
	...
	private TipoCodigoExpediente getTipoCodigoExpediente(Hashtable htEJGs) throws ClsExceptions {
		...
		codigoExpediente.setNumExpediente(numExpediente);		
		Integer anyoExpediente = SIGAServicesHelper.getInteger("año del expediente", (String)htEJGs.get(DE_CE_ANYOEXPEDIENTE));		
		if (anyoExpediente != null) {
			codigoExpediente.setAnyoExpediente(anyoExpediente);	
		}
	...
	private TipoCodigoExpedienteServicio getTipoCodigoExpedienteServicio(Hashtable htEJGs) {
		...
		Integer anyoExpServicio = SIGAServicesHelper.getInteger("año CAJG" ,(String)htEJGs.get(DE_CES_ANYOEXPEDIENTESERVICIO));
	...
	private void rellenaPretension(Expediente expediente, Hashtable htEJGs) throws Exception {
		...
		Integer renunciaDesignacion = SIGAServicesHelper.getInteger("renuncia designación", (String)htEJGs.get(DDJ_RENUNCIADESIGNACION));
	...
	private void datosSolicitante(Expediente expediente, Hashtable htEJGs) throws Exception {
		...
		value = SIGAServicesHelper.getDouble("valoración económica bienes inmuebles", (String) htEJGs.get(DS_DEP_PBI_VALORACIONECONOMICA));
		if (value != null) {
		...
		value = SIGAServicesHelper.getDouble("valoracion económica bienes muebles", (String) htEJGs.get(DS_DEP_PBM_VALORACIONECONOMICA));
		if (value != null) {
		...
		value = SIGAServicesHelper.getDouble("valoración económica otros bienes", (String) htEJGs.get(DS_DEP_PBO_VALORACIONECONOMICA));
		if (value != null) {
	...
	private void datosPersona(DatosPersona datosPersona, Hashtable htEJGs, String dp_tipopersona_cda, String dp_tipoidentificacion_cda,
		...
		Long value = SIGAServicesHelper.getLong("número de hijos", (String)htEJGs.get(dp_numhijos));
		if (value != null) {
	...
	public void execute() throws Exception {
		...
		try {
			...
			ClsLogging.writeFileLog("El parámetro de digitalización de documentos está a "+ isActivoEnvioDigitalizacionDoc, 3);
...
//
//					[…]
//					2021-05-21 13:55:02 Subiendo Expedient econòmic(IDO_2079_GEN_45297819D.pdf) al servidor FTP
