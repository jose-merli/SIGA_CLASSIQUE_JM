...
//			 fechas
	 ...
	 * @throws ClsExceptions
	 */
	private String getPaqueteFacturacionSJCS(String idInstitucion) throws ClsExceptions {
		GenParametrosAdm admParam = new GenParametrosAdm(this.usrbean);
		String sParametroFacturacionHitos = admParam.getValor("" + idInstitucion, "FCS", "FACTURACION_HITOS", "N");
		if (sParametroFacturacionHitos!=null && sParametroFacturacionHitos.equalsIgnoreCase("S")) {
			return "PKG_SIGA_FACT_SJCS_HITOS";
		} else {
			return "PKG_SIGA_FACTURACION_SJCS";
		}	
	}	
	...
	public void ejecutarFacturacion(String idInstitucion, String idFacturacion, UserTransaction tx) throws ClsExceptions, SIGAException {
		...
		try {
			...
			if (v!=null && v.size()>0) {
				...
				// Obtiene el paquete con el que realiza la facturacion SJCS
				String sPaqueteFacturacionSJCS = this.getPaqueteFacturacionSJCS(idInstitucion);
...
//			SIGASvlProcesoAutomaticoRapido.NotificarAhora(SIGASvlProcesoAutomaticoRapido.procesoRapidoFacturacion);
	...
	public void borrarFacturacion(String idInstitucion, String idFacturacion, UsrBean usr) throws SIGAException, ClsExceptions
	{
		...
		resultadoPl = ClsMngBBDD.callPLProcedure("{call " + sPaqueteFacturacionSJCS + ".PROC_FCS_BORRAR_FACTURACION (?,?,?,?)}", 2, param_in);
