...
//	}
	...
	public Hashtable getConceptosPendientesYTotal(Integer idInstitucion, Integer idFacturacion) throws ClsExceptions{
		...
		consulta.append(" decode(SUM(P.IMPORTEOFICIO),  0, 0, round( SUM(P.IMPORTEOFICIO)  * 100 / max(F.IMPORTEOFICIO)  ,2))  as TOTALPORCENTAJEPAGADOOFICIO, ");
		consulta.append(" decode(SUM(P.IMPORTEGUARDIA), 0, 0, round( SUM(P.IMPORTEGUARDIA) * 100 / max(F.IMPORTEGUARDIA) ,2))  as TOTALPORCENTAJEPAGADOGUARDIA, ");
		consulta.append(" decode(SUM(P.IMPORTEEJG),     0, 0, round( SUM(P.IMPORTEEJG)     * 100 / max(F.IMPORTEEJG)     ,2))  as TOTALPORCENTAJEPAGADOEJG, ");
		consulta.append(" decode(SUM(P.IMPORTESOJ),     0, 0, round( SUM(P.IMPORTESOJ)     * 100 / max(F.IMPORTESOJ)     ,2))  as TOTALPORCENTAJEPAGADOSOJ, ");
