			...
			for (int k = 0; k < alClavesDestinatario.size(); k++) {//Para cada destinatario se recorrer sus claves de itreacion
				...
				for (int i = 0; i < vPlantillasInforme.size(); i++) {//Por ultimo se recorren las posible plantillas de cada registro de destinatario
					...
					if(programInfBean.getIdTipoInforme().equals(EnvioInformesGenericos.comunicacionesDesigna)){
	                    ...
	                    Vector designaVector = designaAdm.getDatosSalidaOficio(idInstitucion, idTurnoDesigna, anioDesigna, numeroDesigna, null, isSolicitantes,isContrarios, null, "1","ES",tipoDestinatarioInforme,agregarEtiqEJG,longitudNumEjg);
...
//	                        listObjetosTelematicos.add(ecomDesignaprovisional);
	                    ...
	                    Vector ejgVector = ejgAdm.getDatosInformeEjg(usrBean.getLocation(), idTipoEJG, anioEJG, numeroEJG, false,false, null,null,true,AdmInformeBean.TIPODESTINATARIO_SCSJUZGADO,agregarEtiqDesigna,longitudNumEjg);
				...
				try {
					...
					if (idTipoEnvio.toString().equals(
						...
						try {
							...
							this.enviarInformeGenericoOrdinario(usr,
											vDestinatarios,
...
//							UserTransaction transaction = usr.getTransactionPesada();
							try {
								usr.setUserName(String.valueOf(programInfGenericoBean.getUsuMod()));
//								transaction.begin();
								this.enviarInformeGenerico(usr,
										destProgramInfBean,
...
//								transaction.rollback();
						...
						try {
							...
							if(destProgramInformesBeanOrdinarios.size()>0){
								...
								this.enviarInformeGenericoOrdinario(usr,destProgramInformesBeanOrdinarios,	programInfGenericoBean,vPlantillas, 
...
//			ReadProperties rp = new ReadProperties("SIGA.properties");	
	...
	public void gestionarComunicacionOrdenDomiciliacion(DefinirEnviosForm form,
			Locale locale, UsrBean userBean) throws ClsExceptions,
			SIGAException {
		...
		if (idPersona != null && enviosHashtable.size() == 1) {
			...
			Vector documentoVector =this.getDocumentosAEnviar(datosInforme,
					informesVector, userBean,
					EnvioInformesGenericos.docDocument,
					EnvioInformesGenericos.comunicacionesOrdenDomicializacion);
	...
	public void gestionarComunicacionAnexosOrdenDomiciliacion(DefinirEnviosForm form,	Locale locale, UsrBean userBean)throws ClsExceptions,
			SIGAException  {
		// Obtenemos la información pertinente relacionada
				MasterReport masterReport = new MasterReport();
				Vector datosInformeVector = null;
				if(form.getIdTipoInforme()!=null && !form.getIdTipoInforme().equals(""))
					...
					if (idPersona != null && enviosHashtable.size() == 1) {
						...
						Vector documentoVector =this.getDocumentosAEnviar(datosInforme,
								informesVector, userBean,
								EnvioInformesGenericos.docDocument,
								EnvioInformesGenericos.comunicacionesAnexoOrdenDomiciliacion);
						if(documentoVector==null)
							throw new SIGAException("messages.envios.error.noExisteDocumento");
						vDocumentos.addAll(documentoVector);
