...
/**
 * 
 * Se crea esta clase para gestionar las comunicaciones masivas Esta intimamente relacionada con DefinirEnviosAction e InformesGenercosAction.
 * 
 * 
 * @author jorgeta
 * 
...
public class EnvioInformesGenericos extends MasterReport {
	...
	/**
	 * En este metodo vamos ha invocar el acceso a datos de cada uno de los informes que se van a generar. 
	 * El hashtable que devuleve tendra dos elemento (row y region). 
	 * region( masterWord.sustituyeRegionDocumento)-->El elemento es un vector de hastable cuyas claves seran los nombres de la region. 
	 ...
	 */
	private Hashtable getDatosInformeFinal(Hashtable datosInforme, UsrBean usrBean) throws ClsExceptions, SIGAException {
		...
		if (idTipoInforme.equals(EnvioInformesGenericos.comunicacionesCenso)) {
			...
			case 0: // Caso de No Colegiados
				total = noColegiadoAdm.getInformeNoColegiadoInforme(idInstitucion, idPersona, idioma, true);
				break;
			...
			datosCVAdm.updateInformeDatosCV(usrBean, Integer.parseInt(idInstitucion), Long.parseLong(idPersona), htDatosInforme);
			...
			Vector vDatosInforme = facturaAdm.selectFacturasMoroso(idInstitucion, idPersona, null, null, alFacturasPersona, null, false, true, usrBean.getLanguage());

			Hashtable htCabeceraInforme = new Hashtable();

			CenClienteAdm admCli = new CenClienteAdm(usrBean);

			Hashtable htCol = admCli.obtenerDatosColegiadoONo(true, idInstitucion.toString(), idPersona, usrBean.getLanguage());
			...
			if (htCol == null) {
				htCol = admCli.obtenerDatosColegiadoONo(false, idInstitucion.toString(), idPersona, usrBean.getLanguage());
			}
			...
			if ((direccion.equals("")) && (codPostal.equals("")) && (localidad.equals("")) && (provincia.equals(""))) {
				...
				direcciones = enviosAdm.getDirecciones(String.valueOf(idInstitucion), idPersona, "2");
			...
			helperInformes.completarHashSalida(htCabeceraInforme, helperInformes.ejecutaFuncionSalida(htFuncion, "PKG_SIGA_FECHA_EN_LETRA.F_SIGA_FECHACOMPLETAENLETRA", "FECHAMESLETRA"));
			...
			htCabeceraInforme.put("BRUTO", UtilidadesNumero.formatoCartas(UtilidadesNumero.redondea(importeTotal, 2)));
			htCabeceraInforme.put("LIQUIDO", UtilidadesNumero.formatoCartas(UtilidadesNumero.redondea(deudaTotal, 2)));
			...
			boolean isSolicitantes = aSolicitantes != null && aSolicitantes.equalsIgnoreCase("S");
			boolean isContrarios = aContrarios != null && aContrarios.equalsIgnoreCase("S");
			...
			Vector datosconsulta = scsDesignaAdm.getDatosSalidaOficio(idinstitucion, idTurno, anio, numero, null, isSolicitantes, isContrarios, 
					idPersonaJG, idioma, idiomaExt, tipoDestinatarioInforme, agregarEtiqEJG, longitudNumEjg);
		...
		} else if (idTipoInforme.equals(EnvioInformesGenericos.comunicacionesEjg) || idTipoInforme.equals(EnvioInformesGenericos.comunicacionesCAJG)) {
			...
			Vector datosconsulta = ejgAdm.getDatosInformeEjg(usrBean.getLocation(), idTipoEJG, anio, numero, isSolicitantes, isAcontrarios, 
					idPersona, tipoDestinatario, isGenerarInformeSinDireccion, tipoDestinatarioInforme, agregarEtiqDesigna, longitudNumEjg);
			...
			if (datosconsulta.size() > 0) {
				...
				for (int i = 0; i < datosconsulta.size(); i++) {
					...
					if (htIdiomasUF.containsKey(idioma)) {
						...
						regionUF = ejgAdm.getDatosRegionUF(usrBean.getLocation(), idTipoEJG, anio, numero, idioma, longitudNumEjg != null ? longitudNumEjg : "5");
						regionConyuge = ejgAdm.getDatosRegionConyuge(usrBean.getLocation(), idTipoEJG, anio, numero, idioma, longitudNumEjg != null ? longitudNumEjg : "5");
		...
		} else if (idTipoInforme.equals(EnvioInformesGenericos.comunicacionesOrdenDomicializacion)) {

			String idPersona = (String) datosInforme.get("idPersona");
			String idMandato = (String) datosInforme.get("idMandato");
			String idCuenta = (String) datosInforme.get("idCuenta");
			String refMandatoSEPA = (String) datosInforme.get("refMandatoSEPA");
		...
		} else if (idTipoInforme.equals(EnvioInformesGenericos.comunicacionesAnexoOrdenDomiciliacion)) {
		...
		} else if (idTipoInforme.equals(EnvioInformesGenericos.comunicacionesSancionesLetrado)) {
			CenSancionAdm sancionAdm = new CenSancionAdm(usrBean);
			String idPersona = (String) datosInforme.get("idPersona");
			String idSancion = (String) datosInforme.get("idSancion");
			datosInforme = sancionAdm.getDatosInformeSancionLetrado(usrBean,idPersona,idInstitucion,idSancion);
			htDatosInforme.put("row", datosInforme);
		}
	...
	/**
	 * Devuelve un Vector con los documentos a Enviar(o descargar) ya generados en formato File o Document dependiendo del parametro de entrada tipodocumento. 
	 * Generara un documento por cada una de las plantillas del parametro vPlantillas(AdmInformeBean)
	 * 
	 * 
	 * @param datosInforme
	 * @param vPlantillas
	 ...
	 * @throws SIGAException
	 */
	public Vector getDocumentosAEnviar(Hashtable datosInforme, Vector vPlantillas, UsrBean usrBean, int tipoDocumento, String tipoComunicacion) throws ClsExceptions, SIGAException {
//		
		String idInstitucion = (String) datosInforme.get("idinstitucion");
		...
		String idiomaExt = (String) datosInforme.get("idiomaExt");
		/*
		 * if (idiomaExt == null || idiomaExt.equals("")) idiomaExt = usrBean.getLanguageExt(); datosInforme.put("idiomaExt", idiomaExt);
		 */
		...
		for (int i = 0; i < vPlantillas.size(); i++) {
			...
			if (tipoComunicacion.equals(EnvioInformesGenericos.comunicacionesListadoGuardias)) {
				...
				if (datosFormulario != null) { // Descarga de fichero
					...
					if (isSolicitantes) {// Descarga de ficheros
						...
						for (int k = 0; k < datosFormulario.size(); k++) {
							...
							HelperInformesAdm helperInformesAdm = new HelperInformesAdm();
							helperInformesAdm.setIdiomaInforme((String) datoInform.get("idInstitucion"), (String) datoInform.get("idPersona"), 
									AdmInformeBean.TIPODESTINATARIO_CENPERSONA, datoInform, usrBean);
							...
							htDatoInforme = getDatosInformeFinal(datoInform, usrBean);
							...
							File fileDocumento = getInformeGenerico(beanInforme, htDatoInforme, (String) datoInform.get("idiomaExt"), identificador.toString(), usrBean, tipoPlantillaWord);
						...
						File fileDocumento = getInformeGenerico(beanInforme, htDatosInformeFinal, (String) datosInforme.get("idiomaExt"), 
								identificador.toString(), usrBean, tipoPlantillaWord);
						String pathDocumento = fileDocumento.getPath();
						// Creacion documentos
						int indice = pathDocumento.lastIndexOf(ClsConstants.FILE_SEP);
						String descDocumento = "";
					...
					datos.put("aSolicitantes", beanInforme.getASolicitantes());
					datos.put("idTipoInforme", tipoComunicacion);
					helperInformesAdm.setIdiomaInforme((String) datos.get("idInstitucion"), (String) datos.get("idPersona"), 
							AdmInformeBean.TIPODESTINATARIO_CENPERSONA, datos, usrBean);
			...
			} else if (tipoComunicacion.equals(EnvioInformesGenericos.comunicacionesDesigna)) {
				...
				if (añadir) {
					...
					if (tipoDestinatario.equalsIgnoreCase(AdmInformeBean.TIPODESTINATARIO_SCSPERSONAJG)) {
						...
						Vector solicitantesVector = designaAdm.getDefendidosDesignaInforme(idInstitucion, anio, idTurno, numero, idPersonaJG, longitudNumEjg);
						...
						for (int j = 0; j < solicitantesVector.size(); j++) {
							...
							if (hashConsultasHechas.containsKey(keyConsultasHechas)) {
								...
								htDatosInformeFinal = getDatosInformeFinalOficio(datosInforme, datosInformeSeleccionado, longitudNumEjg, usrBean);
							...
							for (int k = 0; k < datosInformeDesigna.size(); k++) {
								...
								fileDocumento = getInformeGenerico(beanInforme, htDatosInforme, idiomaExt, identificador.toString(), usrBean, tipoPlantillaWord);
			...
			} else if (tipoComunicacion.equals(EnvioInformesGenericos.comunicacionesEjg) || tipoComunicacion.equals(EnvioInformesGenericos.comunicacionesCAJG) 
					|| tipoComunicacion.equals(EnvioInformesGenericos.comunicacionesResolucionEjg)) {
				String tipoDestinatarioEnvio = (String) datosInforme.get("tipoDestinatario");
				String tipoDestinatario = beanInforme.getDestinatarios();
				boolean añadir = tipoDestinatarioEnvio == null;
				if (!añadir) {
					if (String.valueOf(tipoDestinatario).equalsIgnoreCase(AdmInformeBean.TIPODESTINATARIO_CENPERSONA)) {
						añadir = tipoDestinatarioEnvio.equalsIgnoreCase(EnvDestinatariosBean.TIPODESTINATARIO_CENPERSONA);

					} else if (String.valueOf(tipoDestinatario).equalsIgnoreCase(AdmInformeBean.TIPODESTINATARIO_SCSPERSONAJG)) {
						añadir = tipoDestinatarioEnvio.equalsIgnoreCase(EnvDestinatariosBean.TIPODESTINATARIO_SCSPERSONAJG);
				...
				if (añadir) {
					...
					if (idInstitucion != null && !"".equals(idInstitucion) && idTipoEJG != null && !"".equals(idTipoEJG) && anio != null 
							&& !"".equals(anio) && id != null && !"".equals(id)) {
					...
					String keyConsultasHechas = idInstitucion + anio + idTipoEJG + numero + isSolicitantes + isAContrarios + isGenerarInformesSindireccion + tipoDestinatario;
					...
					if (hashConsultasHechas.containsKey(keyConsultasHechas)) {
						...
						htDatosInformeFinal = getDatosInformeEjgFinal(datosInforme, datosInformeSeleccionado, longitudNumEjg, usrBean);
					...
					if (datosInformeEjg != null && datosInformeEjg.size() > 0) {
						...
						for (int k = 0; k < datosInformeEjg.size(); k++) {
							...
							// jbd // R1406_0080
							/*
							 * identificador.append(idTipoEJG); identificador.append("_");
							 */
			...
			} else if (tipoComunicacion.equals(EnvioInformesGenericos.comunicacionesExpedientes)) {
...
//							descDocumento = pathDocumento.substring(indice + 1);
			...
			} else if (tipoComunicacion.equals(EnvioInformesGenericos.comunicacionesOrdenDomicializacion)
					|| tipoComunicacion.equals(EnvioInformesGenericos.comunicacionesAnexoOrdenDomiciliacion)) {
				...
				for (Hashtable row : listDatosInforme) {
					ident = new StringBuffer((String) row.get("REFMANDATOSEPA"));
					Hashtable htDatosInforme = new Hashtable();
					htDatosInforme.put("row", row);
					File fileDocumento = getInformeGenerico(beanInforme, htDatosInforme, idiomaExt, ident.toString(), usrBean, tipoPlantillaWord);
					String pathDocumento = fileDocumento.getPath();
					// Creacion documentos
					int indice = pathDocumento.lastIndexOf(ClsConstants.FILE_SEP);
					String descDocumento = "";
				...
				if (tipoComunicacion.equals(EnvioInformesGenericos.comunicacionesPagoColegiados)) {
					...
					String hoy = UtilidadesString.formatoFecha(new Date(), "yyyyMMddhhmmssSSS");
					identificador.append(hoy);
					fileDocumento = getInformeGenerico(beanInforme, htDatosInformeFinal, idiomaExt, identificador.toString(), usrBean, tipoPlantillaFo);

				} else if (tipoComunicacion.equals(EnvioInformesGenericos.comunicacionesFacturaRectificativa)) {
					// String nColegiado = (String) datosInforme.get("NCOLEGIADO");
					// String idPago = (String) datosInforme.get("IDPAGOS");
					String idTipoInforme = (String) datosInforme.get("idTipoInforme");
					identificador = new StringBuffer();
	 ...
	 * Por definicion de requisitos se generara UN UNICO ENVIO(EnvEnvios) cuyos destinatarios sera cada uno de los destinatarios programados y 
	 * los documentos seran la suma de todos los docuemntos de cada uno de los destinatarios
	 * 
	 * @param usrBean
	...
	public void enviarInformeGenericoOrdinario(UsrBean usrBean, Vector vDestProgramInfBean, EnvProgramInformesBean programInfBean, Vector vPlantillasInforme, EnvEnvioProgramadoBean envioProgramadoBean) throws ClsExceptions, SIGAException {
		...
		for (int j = 0; j < vDestProgramInfBean.size(); j++) {
			...
			if (destProgramInfBean.getTipoDestinatario().equals(EnvDestinatariosBean.TIPODESTINATARIO_CENPERSONA)) {
				...
				datosInforme.put("idPersona", destProgramInfBean.getIdPersona().toString());
			} else if (destProgramInfBean.getTipoDestinatario().equals(EnvDestinatariosBean.TIPODESTINATARIO_SCSCONTRARIOSJG)) {
				datosInforme.put("idPersona", destProgramInfBean.getIdPersona().toString());
			}

			datosInforme.put("idInstitucion", destProgramInfBean.getIdInstitucionPersona().toString());
					...
					if (programInfBean.getIdTipoInforme().equals(EnvioInformesGenericos.comunicacionesMorosos)) {
						...
						if (programInfBean.getIdTipoInforme().equals(EnvioInformesGenericos.comunicacionesDesigna)) {
							...
							designas.add(designa);
								...
								|| programInfBean.getIdTipoInforme().equals(EnvioInformesGenericos.comunicacionesCAJG)) {
			...
			if (destProgramInfBean.getTipoDestinatario().equals(EnvDestinatariosBean.TIPODESTINATARIO_CENPERSONA)) {
				...
				if (htProcuradores.containsKey(idPersona)) {
					Vector vAuxDocumentos = (Vector) htProcuradores.get(idPersona);
					vDocumentos.addAll(vAuxDocumentos);
				...
				if (htJuzgado.containsKey(idPersona)) {
					Vector vAuxDocumentos = (Vector) htJuzgado.get(idPersona);
					vDocumentos.addAll(vAuxDocumentos);
		...
		if (programInfBean.getIdTipoInforme().equals(EnvioInformesGenericos.comunicacionesMorosos)) {
			for (int i = 0; i < lComunicacionesMorosos.size(); i++) {
				...
				envio.generarComunicacionMoroso(comunicacion.getIdPersona().toString(), comunicacion.getDocumentos(), comunicacion.getFacturas(), 
	...
	public void enviarInformeGenericoTelematico(UsrBean usrBean, EnvDestProgramInformesBean destProgramInfBean, EnvProgramInformesBean programInfBean, 
			Vector vPlantillasInforme, EnvEnvioProgramadoBean envioProgramadoBean) throws ClsExceptions, SIGAException {
		...
		for (int k = 0; k < alClavesDestinatario.size(); k++) {// Para cada destinatario se recorrer sus claves de itreacion
			...
			for (int i = 0; i < vPlantillasInforme.size(); i++) {// Por ultimo se recorren las posible plantillas de cada registro de destinatario
				...
				if (programInfBean.getIdTipoInforme().equals(EnvioInformesGenericos.comunicacionesDesigna)) {
					...
					Vector designaVector = designaAdm.getDatosSalidaOficio(idInstitucion, idTurnoDesigna, anioDesigna, 
							numeroDesigna, null, isSolicitantes, isContrarios, null, "1", "ES", tipoDestinatarioInforme, agregarEtiqEJG, longitudNumEjg);
					...
					Vector ejgVector = ejgAdm.getDatosInformeEjg(usrBean.getLocation(), idTipoEJG, anioEJG, numeroEJG, false, false,
							null, null, true, AdmInformeBean.TIPODESTINATARIO_SCSJUZGADO, agregarEtiqDesigna, longitudNumEjg);
	...
	public void enviarInformeGenericoSms(UsrBean usrBean, EnvDestProgramInformesBean destProgramInfBean, 
			EnvProgramInformesBean programInfBean, Vector vPlantillasInforme, EnvEnvioProgramadoBean envioProgramadoBean) throws ClsExceptions, SIGAException {
	...
	public void enviarInformeGenerico(UsrBean usrBean, EnvDestProgramInformesBean destProgramInfBean, EnvProgramInformesBean programInfBean, Vector vPlantillasInforme, EnvEnvioProgramadoBean envioProgramadoBean) throws ClsExceptions, SIGAException {
		...
		if (destProgramInfBean.getTipoDestinatario().equals(EnvDestinatariosBean.TIPODESTINATARIO_CENPERSONA)) {
			...
			datosInforme.put("idContrario", destProgramInfBean.getIdPersona().toString());
			// ATENCION EN INFORMES DE EXPEDIENTES USAMOS LAS CONSTANTES DE SOLICITANTES EJG COMO DENUNCIANTES, QUE SON CLIENTES
		...
		if (programInfBean.getIdTipoInforme().equals(EnvioInformesGenericos.comunicacionesExpedientes)) {
			...
			if (alClavesDestinatario == null) {
				...
				for (int i = 0; i < alClavesDestinatario.size(); i++) {
					...
					Envio.generarComunicacionExpediente((String) htClaves.get("idInstitucion"), new Integer((String) htClaves.get("idInstitucionTipoExp")), 
							new Integer((String) htClaves.get("idTipoExp")), new Integer((String) htClaves.get("numeroExpediente")), 
		...
		if (programInfBean.getIdTipoInforme().equals(EnvioInformesGenericos.comunicacionesMorosos)) {
			ArrayList alFacturas = (ArrayList) datosInforme.get("idFacturas");
			envio.generarComunicacionMoroso(destProgramInfBean.getIdPersona().toString(), vDocumentos, alFacturas, programInfBean.getIdInstitucion().toString(), enviosBean.getDescripcion());
		}
	...
	public Vector getPlantillasInforme(String plantillas, String separador, UsrBean usr) throws ClsExceptions, SIGAException {
		...
		for (int i = 0; i < d.length; i++) {
			...
			AdmInformeBean beanInforme = admInforme.obtenerInforme(idsPlantilla[1], idsPlantilla[0]);
	...
	public Vector<AdmInformeBean> getInformes(List informesList, UsrBean usr) throws ClsExceptions, SIGAException {
		Vector vPlantillas = new Vector();
		AdmInformeAdm admInforme = new AdmInformeAdm(usr);
		for (int i = 0; i < informesList.size(); i++) {
			String[] idsInforme = ((String) informesList.get(i)).split(",");
			AdmInformeBean beanInforme = admInforme.obtenerInforme(idsInforme[1], idsInforme[0]);
			vPlantillas.add(beanInforme);
	...
	// GENEXP@@GENEXP2
	/**
	 * Metodo que nos convierte una cadena de caracteres separados(pepe||juan||pedro) por un string en un vector de datos([pepe,juan,pedro])
	 * 
	 * @param cadenaUnida
	 * @param separador
	 * @return
	...
	public Vector getDatosSeparados(String cadenaUnida, String separador) throws ClsExceptions, SIGAException {
	...
	/**
	 * Metodo que nos incluye un vector de plantillas transformado en texto plano y nos lo incluye en el texto plano a tratar delos datos a enviar
	 * 
	 * @param form
	 * @param vPlantillas
	 * @return
	...
	private String getDatosAEnviar(InformesGenericosForm form, Vector vPlantillas) throws SIGAException, Exception {
		...
		while (st1.hasMoreTokens()) {
			...
			if (registro != null && !registro.equals("")) {
				...
				if (form.getTipoPersonas() != null && !form.getTipoPersonas().equals("")) {
					datosEnvios.append("##");
	...
	/**
	 * Funcion principal de la clase que nos gestiona el envio o descarga de los informes genericos. En caso de que sea descargar es el metodo que se encarga que generar y empaquetar los archivos necesarios. 
	 * En el caso de que sea enviar, es el encargado de preparar los datos que mas tarde se manejaran. Este metodo se invocara desde InformesGenericosAction y generara los informes o preparada los datos 
	 * para los futuros envios. En este caso el encargado de invocar la genetacion de envios sera DefinirEnviosAction
	 */
	public File getInformeGenerico(InformesGenericosForm informeGenerico, String idsesion, UsrBean usr, boolean isEnviar, boolean isPermisoEnvio) throws SIGAException, Exception {
		...
		if (isEnviar && isPermisoEnvio) { // se prepara el envio, pero nada mas
			setDatosEnvios(getDatosAEnviar(informeGenerico, getDatosSeparados(informeGenerico.getIdInforme(), "##")));
			ficheroSalida = null;
		} else { // se generan los ficheros para descargar, comprueba cpago
					// cuando se accede por medio de abonos a las cartas de pago
			String datos = getDatosAEnviar(informeGenerico, getDatosSeparados(informeGenerico.getIdInforme(), "##"));
			...
			if (claveIterante != null && !claveIterante.equals("")) {
				datosFormulario = setCamposIterantes(datosFormulario, claveIterante);
			}
			...
			if (idTipoInforme.equals(EnvioInformesGenericos.comunicacionesListadoGuardias)) {
				...
				if (datosInforme != null) {
					...
					if (vPlantillas == null) {
						vPlantillas = getPlantillasInforme(informeGenerico.getIdInforme(), "##", usr);
					}
					informesRes.addAll(this.getDocumentosAEnviar(datosTotales, vPlantillas, usr, EnvioInformesGenericos.docFile, idTipoInforme));
				}
			} else {
				for (int j = 0; j < datosFormulario.size(); j++) {
					...
					if (datosInforme != null) {
						...
						Vector datosVector = this.getDocumentosAEnviar(datosInforme, vPlantillas, usr, EnvioInformesGenericos.docFile, idTipoInforme);
						if (datosVector != null)
			...
			if (informesRes.size() == 0)
				...
				AdmTipoInformeBean beanT = admT.obtenerTipoInforme(informeGenerico.getIdTipoInforme());
				...
				String nombreFicheroZIP = UtilidadesBDAdm.getFechaCompletaBD("").replaceAll("/", "").replaceAll(":", "").replaceAll(" ", "").replaceAll("-", "");
	...
	/**
	 * Metodo que nos agrupa en un vector de hastable los datos por los que itera. Se incluira en un elemento de este hashtable de tipo ArtrayList y con la clave que es el PLURAL 
	 * de la clave iterante [idpersona=1,idFactura= 22],[idPersona=1,idFactura=33]-->{idPersona=1,[idFactura=2,idFactura=33]}
	 * 
	 * @param vCampos
	 * @param claveIterante
	 * @return
	 * @throws ClsExceptions
	 */
	public Vector setCamposIterantes(Vector vCampos, String claveIterante) throws ClsExceptions {
		Hashtable htPersona = new Hashtable();
	...
	private Vector setCamposIterantes(Vector vCampos, ArrayList clavesIterante, String nombre) throws ClsExceptions {
		Hashtable htPersona = new Hashtable();
		ArrayList alClaves = null;
		Vector vCamposConArray = new Vector();
		Map htClave = null;
	...
	private File getInformeGenerico(AdmInformeBean beanInforme, Hashtable htDatosInforme, String idiomaExt, String identificador, UsrBean usr, int tipoPlantillaInforme) throws SIGAException, ClsExceptions {
		File f = null;
		Hashtable htDatosInformeClone = (Hashtable) htDatosInforme.clone();
		switch (tipoPlantillaInforme) {
		case tipoPlantillaWord:
			f = getInformeGenericoWord(beanInforme, htDatosInformeClone, idiomaExt, identificador, usr);
			break;
		case tipoPlantillaFo:
			if (beanInforme.getIdTipoInforme().equals(comunicacionesPagoColegiados)) {
				...
				f = infColegiadoFo.getInformeGenericoFo(beanInforme, htDatosInformeClone, idiomaExt, identificador, usr);
				...
				htDatosInformeClone = (Hashtable) htDatosInformeClone.get("row");
				f = infColegiadoFo.getInformeGenericoFo(beanInforme, htDatosInformeClone, idiomaExt, identificador, usr);
			} else if (beanInforme.getIdTipoInforme().equals(comunicacionesFacturaRectificativa)) {
				InformeAbono informeAbono = new InformeAbono();
				htDatosInformeClone = (Hashtable) htDatosInformeClone.get("row");
				f = informeAbono.getInformeGenericoFo(beanInforme, htDatosInformeClone, idiomaExt, identificador, usr);
			}
			break;
	...
	public void setPersonasExpedientes(Vector vCampos, UsrBean userBean) throws ClsExceptions, SIGAException {
		...
		for (int i = 0; i < vCampos.size(); i++) {
			...
			if (contrarios != null) {
				for (int j = 0; j < contrarios.size(); j++) {
					Hashtable personaHashtable = (Hashtable) contrarios.get(j);
					String idPersonaJG = (String) personaHashtable.get("IDPERSONA");
	...
	public void gestionarComunicacionCenso(DefinirEnviosForm form, Locale locale, UsrBean userBean) throws ClsExceptions, SIGAException {
		...
		if (idPersona != null && enviosHashtable.size() == 1) {
			...
			vDocumentos.addAll(this.getDocumentosAEnviar(datosInforme, informesVector, userBean, EnvioInformesGenericos.docDocument, EnvioInformesGenericos.comunicacionesCenso));
			...
			if (datosInformeVector.size() > 0) {
				...
				if (!isInformeProgramado) {
					...
					for (int j = 0; j < informesVector.size(); j++) {
						...
						informesBean.setIdInstitucion(informeBean.getIdInstitucion());
						informesGenericoAdm.insert(informesBean);
	...
	public void gestionarComunicacionPagoColegiados(DefinirEnviosForm form, Locale locale, UsrBean userBean) throws ClsExceptions, SIGAException {
		...
		if (idPersona != null && enviosHashtable.size() == 1) {
			...
			for (int i = 0; i < datosInformeVector.size(); i++) {
				...
				vDocumentos.addAll(this.getDocumentosAEnviar(datosInforme, informesVector, userBean, EnvioInformesGenericos.docDocument, EnvioInformesGenericos.comunicacionesPagoColegiados));
	...
	public void gestionarComunicacionFacturasRectificativas(DefinirEnviosForm form, Locale locale, UsrBean userBean) throws ClsExceptions, SIGAException {
		...
		for (int i = 0; i < datosInformeVector.size(); i++) {
			Hashtable datosInforme = (Hashtable) datosInformeVector.get(i);
			vDocumentos.addAll(this.getDocumentosAEnviar(datosInforme, informesVector, userBean, EnvioInformesGenericos.docDocument, EnvioInformesGenericos.comunicacionesFacturaRectificativa));
	...
	public void gestionarComunicacionFacturacionColegiados(DefinirEnviosForm form, Locale locale, UsrBean userBean) throws ClsExceptions, SIGAException {
		...
		if (idPersona != null && enviosHashtable.size() == 1) {
			...
			while (iteEnvios.hasNext()) {
				...
				Vector auxDatosInformeVector = this.setCamposIterantes(datosInformeVector, alClavesPago, "idFacturaciones");
	...
	public void gestionarComunicacionMorosos(DefinirEnviosForm form, Locale locale, UsrBean userBean) throws ClsExceptions, SIGAException {
		...
		if (idPersona != null && enviosHashtable.size() == 1) {
			...
			if (claveIterante != null) {
				datosInformeVector = this.setCamposIterantes(datosInformeVector, claveIterante);
			}
			...
			for (int i = 0; i < datosInformeVector.size(); i++) {
				...
				// en el metodo getDatosInformeFinal metemos la key definitiva
				// idFacturas en el datosInforme
				vDocumentos.addAll(this.getDocumentosAEnviar(datosInforme, informesVector, userBean, EnvioInformesGenericos.docDocument, EnvioInformesGenericos.comunicacionesMorosos));
			...
			datosInformeVector = this.setCamposIterantes(datosInformeVector, alClavesFactura, "idFacturas");
			while (iteEnvios.hasNext()) {
				keyEnvios = (String) iteEnvios.next();
				informesList = enviosHashtable.get(keyEnvios);
				informesVector = this.getInformes(informesList, userBean);
	...
	public Hashtable getDatosSalidaListaGuardias(Hashtable datosInforme, UsrBean usrBean, boolean isSolicitantes) throws ClsExceptions {
		...
		try {
			...
			while (iteradorListaGuardias.hasNext()) {
				...
				String idPersona = UtilidadesString.split(keyListaGuardia.toString(), "|||")[1];
				Vector list = (Vector) datosLista.get(keyListaGuardia);
				int tam = 0;
				CenColegiadoAdm admCol = new CenColegiadoAdm(usrBean);
				// HelperInformesAdm helperInformesAdm = new HelperInformesAdm();
				// helperInformesAdm.setIdiomaInforme(idInstitucion, idPersona, AdmInformeBean.TIPODESTINATARIO_CENPERSONA,datosInforme, usrBean);
				Hashtable htCol = admCol.obtenerDatosColegiado(idInstitucion.toString(), idPersona, (String) datosInforme.get("idioma"));
				// se añade esta linea para que recupere todos los campos de la
				// select que esta en la función obtenerDatosColegiado(..)
				// y posteriormente utilizarla en las plantillas.
				htCabeceraInforme.putAll(htCol);
	...
	private TreeMap getIdColegiados(Hashtable datosInforme, UsrBean usrBean) throws ClsExceptions {
		...
		try {
			...
			while (listaResultados.hasMoreElements()) {
				...
				// Recopilacion datos
				/*
				 * RGG 08/10/2007 cambio para que obtenga todas las guardias del tiron Vector 
				 */
	...
	protected TreeMap calcularPersonasTotal(Vector datos) throws SIGAException {
		...
		try {
			...
			for (int j = 0; j < datos.size(); j++) {
				...
				String key = (GstDate.convertirFecha((String) lineaGuardia.get("FECHA_INICIO"), "yyyy/MM/dd HH:mm:ss").getTime()) + "##" + 
							(String) lineaGuardia.get("ORDEN") + "##" + (String) lineaGuardia.get("GUARDIA") + "##" + (String) lineaGuardia.get("POSICION") 
	...
	public void setPersonasDesignas(Vector vCampos, UsrBean userBean) throws ClsExceptions, SIGAException {
	...
	private void setPersonasDesignas(String datosInforme, UsrBean userBean) throws ClsExceptions, SIGAException {
		ScsDesignaAdm desigAdm = new ScsDesignaAdm(userBean);
		ScsDefendidosDesignaAdm defendidosAdm = new ScsDefendidosDesignaAdm(userBean);
		String idTurno = null;
		String idInstitucion = null;
	...
	public void gestionarComunicacionDesignas(DefinirEnviosForm form, Locale locale, UsrBean userBean) throws ClsExceptions, SIGAException {
		...
		if (isPersonaUnica && enviosHashtable.size() == 1 && !isASolicitantes && !isAProcurador && !isAJuzgado) {
			...
			for (int i = 0; i < datosInformeVector.size(); i++) {
				...
				vDocumentos.addAll(this.getDocumentosAEnviar(datosInforme, informesVector, userBean, EnvioInformesGenericos.docDocument, EnvioInformesGenericos.comunicacionesDesigna));
				...
				for (int j = 0; j < solicitantesVector.size(); j++) {
					...
					if (hashConsultasHechas.containsKey(keyConsultasHechas)) {
						...
						htDatosInformeFinal = getDatosInformeFinalOficio(datosInforme, datosInformeSeleccionado, longitudNumEjg, userBean);
						hashConsultasHechas.put(keyConsultasHechas, htDatosInformeFinal);
					}
								...
								if (idSolicitanteJG != null && !idSolicitanteJG.equals("")) {
									...
									if (String.valueOf(tipoDestinatario[k]).equalsIgnoreCase(AdmInformeBean.TIPODESTINATARIO_CENPERSONA)) {
										...
										if (form.getIsCodigoEjis() != null && form.getIsCodigoEjis().equals("1")) {
											...
											Vector juzgadoVector = juzgadoAdm.busquedaJuzgado(programInformes.getIdInstitucion().toString(), idJuzgado);
	...
	public void gestionarComunicacionEjg(DefinirEnviosForm form, Locale locale, UsrBean userBean) throws ClsExceptions, SIGAException {
		...
		if (isDestinatarioUnico && enviosHashtable.size() == 1 && destinatariosHashtable!=null && destinatariosHashtable.contains(EnvDestinatariosBean.TIPODESTINATARIO_CENPERSONA) && !isASolicitantes && !isAJuzgado && !isAContrario && !isAProcurador) {
			...
			for (int i = 0; i < datosInformeVector.size(); i++) {
				...
				vDocumentos.addAll(this.getDocumentosAEnviar(datosInforme, informesVector, userBean, EnvioInformesGenericos.docDocument, EnvioInformesGenericos.comunicacionesEjg));
					...
					for (int j = 0; j < informesVector.size(); j++) {
						...
						if (tiposDestinatario != null) {
							...
							if (idSolicitanteJG != null && !idSolicitanteJG.equals("")) {
								...
								if (String.valueOf(tiposDestinatario).equalsIgnoreCase(AdmInformeBean.TIPODESTINATARIO_SCSPERSONAJG)) {
								...
								if (String.valueOf(tiposDestinatario).equalsIgnoreCase(AdmInformeBean.TIPODESTINATARIO_CENPERSONA) && personasDesignasEjg != null) {
								...
								} else if (String.valueOf(tiposDestinatario).equalsIgnoreCase(AdmInformeBean.TIPODESTINATARIO_SCSPERSONAJG) && vSolicitantesEjg != null) {
									for (int jta = 0; jta < vSolicitantesEjg.size(); jta++) {
										Hashtable htSolicitante = (Hashtable) vSolicitantesEjg.get(jta);
										String idPersonaJG = (String) htSolicitante.get(ScsDefendidosDesignaBean.C_IDPERSONA);
										destProgramInformes = new EnvDestProgramInformesBean();
									...
									for (int jta = 0; jta < vContrariosEjg.size(); jta++) {
										...
										destProgramInformes.setIdPersona(new Long(idPersonaJG));
										destProgramInformes.setIdInstitucionPersona(new Integer(idInstitucion));
										destProgramInformes.setTipoDestinatario(EnvDestinatariosBean.TIPODESTINATARIO_SCSCONTRARIOSJG);
	...
	public void gestionarComunicacionJustificaciones(DefinirEnviosForm form, Locale locale, UsrBean userBean) throws ClsExceptions, SIGAException {
		...
		if (idPersona != null && enviosHashtable.size() == 1) {
			...
			for (int i = 0; i < datosInformeVector.size(); i++) {
				Hashtable datosInforme = (Hashtable) datosInformeVector.get(i);
				vDocumentos.addAll(this.getDocumentosAEnviar(datosInforme, informesVector, userBean, EnvioInformesGenericos.docDocument, EnvioInformesGenericos.comunicacionesJustificacion));
	...
	public void gestionarComunicacionExpedientes(DefinirEnviosForm form, Locale locale, UsrBean userBean) throws ClsExceptions, SIGAException {
		...
		if (enviosHashtable.size() == 1 && destinatariosHashtable != null && destinatariosHashtable.size() == 1) {
			...
			for (int i = 0; i < datosInformeVector.size(); i++) {
				...
				// generando los documentos para cada persona
				vDocumentos.addAll(this.getDocumentosAEnviar(datosInforme, informesVector, userBean, EnvioInformesGenericos.docDocument, EnvioInformesGenericos.comunicacionesExpedientes));
			...
			while (iteEnvios.hasNext()) {
				...
				for (int i = 0; i < datosInformeVector.size(); i++) {
					...
					for (int j = 0; j < informesVector.size(); j++) {
						...
						if (tiposDestinatario.equalsIgnoreCase(AdmInformeBean.TIPODESTINATARIO_CENPERSONA) && denunciados != null && denunciados.size() > 0) {
							...
							// ATENCION EN INFORMES DE EXPEDIENTES USAMOS LAS CONSTANTES DE SOLICITANTESEJG COMO DENUNCIANTES
						} else if (tiposDestinatario.equalsIgnoreCase(AdmInformeBean.TIPODESTINATARIO_SCSPERSONAJG) && denunciantes != null && denunciantes.size() > 0) {
							
							//Sólo en el caso de que el envio de la plantilla sea para Solicitante Recorrer los datos de envío 
							//cogiendo los denunciantes y ver si alguno tiene la dirección dada de baja, si alguna está dada de baja lanzamos la excepción.
							
									CenDireccionesAdm cenDireccionesAdm = new CenDireccionesAdm(userBean);
									if(denunciantes != null && denunciantes.size() >0 ){
									
										ExpDenuncianteBean denuncianteBean = (ExpDenuncianteBean) denunciantes.get(0);
							...
							for (int jta = 0; jta < denunciantes.size(); jta++) {
								ExpDenuncianteBean denuncianteBean = (ExpDenuncianteBean) denunciantes.get(jta);
								String idPersonaJG = denuncianteBean.getIdPersona().toString();
								destProgramInformes = new EnvDestProgramInformesBean();
								destProgramInformes.setIdProgram(programInformes.getIdProgram());
								destProgramInformes.setIdEnvio(programInformes.getIdEnvio());
								destProgramInformes.setIdInstitucion(programInformes.getIdInstitucion());
								destProgramInformes.setIdPersona(denuncianteBean.getIdPersona());
								destProgramInformes.setIdInstitucionPersona(new Integer(idInstitucion));
								destProgramInformes.setTipoDestinatario(EnvDestinatariosBean.TIPODESTINATARIO_SCSPERSONAJG);
						...
						} else if (tiposDestinatario.equalsIgnoreCase(AdmInformeBean.TIPODESTINATARIO_SCSPROCURADOR) && partes != null && partes.size() > 0) {

							for (int jta = 0; jta < partes.size(); jta++) {
								...
								destProgramInformes.setIdEnvio(programInformes.getIdEnvio());
								destProgramInformes.setIdInstitucion(programInformes.getIdInstitucion());
								destProgramInformes.setIdPersona(new Long(partesBean.getIdPersona()));
								destProgramInformes.setIdInstitucionPersona(new Integer(idInstitucion));
								destProgramInformes.setTipoDestinatario(EnvDestinatariosBean.TIPODESTINATARIO_SCSPROCURADOR);
	...
	private void procesoAutomaticoGeneracionEnvios() throws ClsExceptions {
		...
		EnvProgramInformesAdm admProgramInfGenericos = new EnvProgramInformesAdm(new UsrBean()); // Este usrbean esta controlado que no se
																									// necesita el valor
		// Sacamos los datos de los datos programados
		// pendientes(ClsConstants.DB_FALSE) de
		// todas las instituciones(null)
				...
				try {
					...
					if (idTipoEnvio.equals(EnvTipoEnviosAdm.K_CORREO_ORDINARIO)) {
						...
						try {
							...
							this.enviarInformeGenericoOrdinario(usr, vDestinatarios, programInfGenericoBean, vPlantillas, programInfGenericoBean.getEnvioProgramado());
						} catch (Exception e) {
						...
						for (int j = 0; j < vDestinatarios.size(); j++) {
							...
							try {
								usr.setUserName(String.valueOf(programInfGenericoBean.getUsuMod()));
								// transaction.begin();
								this.enviarInformeGenerico(usr, destProgramInfBean, programInfGenericoBean, vPlantillas, programInfGenericoBean.getEnvioProgramado());
								// transaction.commit();
							} catch (Exception e) {
								// transaction.rollback();

								if (destProgramInfBean.getIdPersona() != null)
									ClsLogging.writeFileLogError(" ----------ERROR ENVIO DE INFORMES GENERICOS PENDIENTES IDPERSONA: " + destProgramInfBean.getIdPersona() + " ", e, 3);
							}
						}
						...
						for (int j = 0; j < vDestinatarios.size(); j++) {
							...
							try {
								usr.setUserName(String.valueOf(programInfGenericoBean.getUsuMod()));
								this.enviarInformeGenericoSms(usr, destProgramInfBean, programInfGenericoBean, vPlantillas, programInfGenericoBean.getEnvioProgramado());
							} catch (Exception e) {
								if (destProgramInfBean.getIdPersona() != null)
								...
								if (destProgramInfBean.getTipoDestinatario().equals(EnvDestinatariosBean.TIPODESTINATARIO_SCSJUZGADO)) {
									...
									if (juzgadoVector != null) {
										...
										if (isCodigoEJIS.equals("1")) {
											// Tiene codigo ejis
											try {
												this.enviarInformeGenericoTelematico(usr, destProgramInfBean, programInfGenericoBean, vPlantillas, programInfGenericoBean.getEnvioProgramado());
									...
									// no esta
									ClsLogging.writeFileLogWithoutSession(" ----------INFO ENVIOS TELEMATICOS SOLO PARA JUZGADOS ", 3);
						...
						try {
							...
							if (destProgramInformesBeanOrdinarios.size() > 0) {
								...
								this.enviarInformeGenericoOrdinario(usr, destProgramInformesBeanOrdinarios, programInfGenericoBean, vPlantillas, programInfGenericoBean.getEnvioProgramado());
							}
						} catch (Exception e) {
							ClsLogging.writeFileLogWithoutSession(" ----------ERROR ENVIO DE INFORMES GENERICOS telematicos de manera ordinaria PENDIENTES " + e.toString(), 3);
						}
	...
	public void gestionarComunicacionCAJG(DefinirEnviosForm form, Locale locale, UsrBean userBean) throws ClsExceptions, SIGAException {
	...
	private Hashtable getDatosInformeFinalOficio(Hashtable datosDesignacion, Hashtable datosInformeSeleccionado, String longitudNumEjg, UsrBean usrBean) throws ClsExceptions, SIGAException {
		...
		Vector datosconsulta = scsDesignaAdm.getDatosSalidaOficio(idinstitucion, idTurno, anio, numero, null, isSolicitantes, 
				isContrarios, idPersonaJG, idioma, idiomaExt, tipoDestinatarioInforme, agregarEtiqEJG, longitudNumEjg);
		if (!isSolicitantes && datosconsulta.size() > 0) {
			Hashtable registro = (Hashtable) datosconsulta.get(0);
			if (registro.get("defendido") != null) {
				htDatosInforme.put("defendido", registro.get("defendido"));
	...
	private Hashtable getDatosInformeEjgFinal(Hashtable datosInforme, Hashtable datosInformeSeleccionado, String longitudNumEjg, UsrBean usrBean) throws ClsExceptions, SIGAException {
		...
		// quitar el idioma de la siguiente consulta, sacarlo abajo
		boolean agregarEtiqDesigna = true;
		Vector datosconsulta = ejgAdm.getDatosInformeEjg(idInstitucion, idTipoEJG, anio, numero, isSolicitantes, isAcontrarios, idPersona, tipoDestinatario, 
				isGenerarInformeSinDireccion, tipoDestinatarioInforme, agregarEtiqDesigna, longitudNumEjg);
		Hashtable<String, Vector> htIdiomasUF = new Hashtable<String, Vector>();
		Hashtable<String, Vector> htIdiomasConyuge = new Hashtable<String, Vector>();
		if (datosconsulta.size() > 0) {
			Hashtable registro = (Hashtable) datosconsulta.get(0);
			...
			for (int i = 0; i < datosconsulta.size(); i++) {
				...
				if (htIdiomasUF.containsKey(idioma)) {
					...
					regionUF = ejgAdm.getDatosRegionUF(idInstitucion, idTipoEJG, anio, numero, idioma, longitudNumEjg != null ? longitudNumEjg : "5");
					regionConyuge = ejgAdm.getDatosRegionConyuge(idInstitucion, idTipoEJG, anio, numero, idioma, longitudNumEjg != null ? longitudNumEjg : "5");
					htIdiomasUF.put(idioma, regionUF);
					htIdiomasConyuge.put(idioma, regionConyuge);
				}
	...
	public void gestionarComunicacionOrdenDomiciliacion(DefinirEnviosForm form, Locale locale, UsrBean userBean) throws ClsExceptions, SIGAException {
		...
		// comprobamos si tiene idPersona, por que si no es asi es que viene de mantenimiento de mandatos y se lo tenemos que meter
		// Sin o viene idPersona se lo tenemos que meter
		Hashtable primerDatosInformeHashtable = (Hashtable) datosInformeVector.get(0);
		String idPersonaPrimerRegistro = (String) primerDatosInformeHashtable.get("idPersona");
		if (idPersonaPrimerRegistro == null) {
			for (int i = 0; i < datosInformeVector.size(); i++) {
				Hashtable ht = (Hashtable) datosInformeVector.get(i);
				CenMandatosCuentasBancariasAdm cuentasBancariasAdm = new CenMandatosCuentasBancariasAdm(userBean);
				CenMandatosCuentasBancariasBean beanMandato = new CenMandatosCuentasBancariasBean();
				String refMandatoSEPA = (String) ht.get("refMandatoSEPA");
				String idInstitucion = (String) ht.get("idInstitucion");
				if (refMandatoSEPA == null || refMandatoSEPA.equals(""))
		...
		if (idPersona != null && enviosHashtable.size() == 1) {
			...
			informesVector = this.getInformes(informesList, userBean);
			Hashtable datosInforme = (Hashtable) datosInformeVector.get(0);
			Vector documentoVector = this.getDocumentosAEnviar(datosInforme, informesVector, userBean, EnvioInformesGenericos.docDocument, EnvioInformesGenericos.comunicacionesOrdenDomicializacion);
			...
			while (iteEnvios.hasNext()) {
				...
				ArrayList<EnvDestProgramInformesBean> destinatariosList = new ArrayList<EnvDestProgramInformesBean>();
				...
				for (int i = 0; i < datosInformeVector.size(); i++) {
					...
					destProgramInformes.setClaves((String) ht.get("refMandatoSEPA"));
					//
					if (!personasList.contains(idPersona)) {
						destProgramInformesAdm.insert(destProgramInformes);
						personasList.add(idPersona);

					}
					destinatariosList.add(destProgramInformes);

				}

				EnvValorCampoClaveBean valorCampoClave = null;
				valorCampoClave = new EnvValorCampoClaveBean();
				for (int k = 0; k < destinatariosList.size(); k++) {
					destProgramInformes = (EnvDestProgramInformesBean) destinatariosList.get(k);
					...
					valorCampoClave.setIdInstitucionPersona(destProgramInformes.getIdInstitucionPersona());
					valorCampoClave.setIdTipoInforme(idTipoInforme);
					valorCampoClave.setClave("refMandatosSEPA");
					valorCampoClave.setCampo("refMandatoSEPA");
					valorCampoClave.setValor(destProgramInformes.getClaves());
					valorCampoClaveAdm.insert(valorCampoClave);
	...
	public void gestionarComunicacionAnexosOrdenDomiciliacion(DefinirEnviosForm form, Locale locale, UsrBean userBean) throws ClsExceptions, SIGAException {
		// Obtenemos la información pertinente relacionada
		MasterReport masterReport = new MasterReport();
		Vector datosInformeVector = null;
		if (form.getIdTipoInforme() != null && !form.getIdTipoInforme().equals(""))
		...
		if (idPersona != null && enviosHashtable.size() == 1) {
			...
			informesVector = this.getInformes(informesList, userBean);
			Hashtable datosInforme = (Hashtable) datosInformeVector.get(0);
			Vector documentoVector = this.getDocumentosAEnviar(datosInforme, informesVector, userBean, EnvioInformesGenericos.docDocument, EnvioInformesGenericos.comunicacionesAnexoOrdenDomiciliacion);
			if (documentoVector == null)
				throw new SIGAException("messages.envios.error.noExisteDocumento");
			vDocumentos.addAll(documentoVector);
