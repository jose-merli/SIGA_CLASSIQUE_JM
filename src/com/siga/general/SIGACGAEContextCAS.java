...
public class SIGACGAEContextCAS {
	...
	public HashMap<String, String> getPermisosFromJWTToken(String token) {
		...
		return (HashMap<String, String>) Jwts.parser().parseClaimsJws(token).getBody().get("permisos");
	}
...
//		ReadProperties rproperties=new ReadProperties("SIGA.properties");
	...
	public UsrBean gerUserFromJWTToken(String token) throws SIGAException {
		...
		String institucion = (String) Jwts.parser().parseClaimsJws(token).getBody().get("institucion");
		String grupo = (String) Jwts.parser().parseClaimsJws(token).getBody().get("grupo");
		HashMap<String, String> permisos = getPermisosFromJWTToken(token);
		List<String> perfiles = (List<String>) Jwts.parser().parseClaimsJws(token).getBody().get("perfiles");
		String[] profiles = (String[]) Jwts.parser().parseClaimsJws(token).getBody().get("perfiles");
		String letrado = (String) Jwts.parser().parseClaimsJws(token).getBody().get("grupo");
