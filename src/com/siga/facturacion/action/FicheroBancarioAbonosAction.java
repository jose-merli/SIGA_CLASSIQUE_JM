				...
				while (listaBancos.hasMoreElements()){
					...
					//Sino es de sjcs hay que buscar los abonos pendientes a través de las facturas de la serie relacionada con el banco y el sufijo que estamos tratando 
						...
						if((banco.get(FacSerieFacturacionBancoBean.C_IDSUFIJO)==null)||banco.get(FacSerieFacturacionBancoBean.C_IDSUFIJO).toString().isEmpty()){
							...
							if((miForm.getIdpropSEPA()!=null)&&(miForm.getIdpropSEPA()!=0)){
									banco.put("IDPROPSEPA", miForm.getIdpropSEPA().toString());
									
							}
								
							if((miForm.getIdpropOtros()!=null)&&(miForm.getIdpropOtros()!=0)){
									banco.put("IDPROPOTROS", miForm.getIdpropOtros().toString());
		...
		try{	
			...
			while(en.hasMoreElements()){
				...
				if (esSEPA) {
					...
					if((bReceptor.getProposito().equals("SALA"))||(bReceptor.getProposito().equals("PENS")))
						registrosBenefSEPA[nRegistrosBenefSEPA].append(bReceptor.getProposito()); //tipo de transferencia
					else
