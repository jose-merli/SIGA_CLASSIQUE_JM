 ...
 * @version david.sanchezp - 30-06-2005: controlar SIGA Exception y
 *   refrescar con metodo exito
...
public class DefinirHitosFacturablesGuardiasAction extends MasterAction
{
		...
		throws SIGAException
	{
		...
		try {
			if (accion != null && accion.equals("consultar"))	
				result = consultar (mapping, miform,
						(String)request.getParameter("tipoConsulta"),
						request, response);
			else
		...
		throws SIGAException
	{
		try {
			...
			ScsTipoAsistenciaColegioAdm tipoAsist = 
				new ScsTipoAsistenciaColegioAdm (this.getUserBean(request));
			String consulta="";
			
			if (tipo.equals("asistencia")) {
				consulta = 
					"select "+UtilidadesMultidioma.getCampoMultidiomaSimple 
								("h."+ScsTipoAsistenciaColegioBean.C_DESCRIPCION,
								usr.getLanguage ())+" TIPOASISTENCIA, " +
					"       decode("+(String)request.getParameter("importeMax")+", " +
					"              0, " +
					"              h."+ScsTipoAsistenciaColegioBean.C_IMPORTE+", " +
					"              h."+ScsTipoAsistenciaColegioBean.C_IMPORTEMAXIMO+") IMPORTE" +
					"  from "+ScsTipoAsistenciaColegioBean.T_NOMBRETABLA+" h " +
					" where h."+ScsTipoAsistenciaColegioBean.C_IDINSTITUCION+" = "+usr.getLocation ();
				tipo= "asistencia";
			}
			else {
				consulta = 
					"select "+UtilidadesMultidioma.getCampoMultidiomaSimple
								("a."+ScsTipoActuacionBean.C_DESCRIPCION, 
								this.getUserBean (request).getLanguage ())+" TIPOACTUACION, " +
					"       decode("+(String) request.getParameter ("importeMax")+", " +
					"              0, " +
					"              a."+ScsTipoActuacionBean.C_IMPORTE+", " +
					"              a."+ScsTipoActuacionBean.C_IMPORTEMAXIMO+") IMPORTE, " +
					"       "+ UtilidadesMultidioma.getCampoMultidiomaSimple
								("b."+ScsTipoAsistenciaColegioBean.C_DESCRIPCION, 
								this.getUserBean (request).getLanguage ())+" TIPOASISTENCIA " +
					"  from "+ScsTipoActuacionBean.T_NOMBRETABLA+" a, " +
					"       "+ ScsTipoAsistenciaColegioBean.T_NOMBRETABLA+" b " +
					" where a."+ScsTipoActuacionBean.C_IDINSTITUCION+" = "+usr.getLocation () +
					"   and a."+ScsTipoActuacionBean.C_IDINSTITUCION+" = " +
					"       b."+ScsTipoAsistenciaColegioBean.C_IDINSTITUCION+
					"   and a."+ScsTipoActuacionBean.C_IDTIPOASISTENCIA+" = " +
					"       b."+ScsTipoAsistenciaColegioBean.C_IDTIPOASISTENCIACOLEGIO;
				tipo= "actuacion";
			}
	 ...
	 * Genera el formulario a partir de los registros de BD para rellenar JSP
	 * 
	 * @param  mapping - Mapeo de los struts
	 * @param  formulario -  Action Form asociado a este Action
	 * @param  request - objeto llamada HTTP 
		...
		throws SIGAException
	{
		...
		try {
			...
			ScsGuardiasTurnoBean guardia = (ScsGuardiasTurnoBean)
					request.getSession ().getAttribute ("DATABACKUPPESTANA");
			Hashtable hash1 = (Hashtable) guardia.getOriginalHash ();
			request.getSession ().setAttribute ("ORIGINALHASH", hash1);
			
			String consulta =
				"select "+UtilidadesMultidioma.getCampoMultidioma 
							("h.descripcion", 
			...
			this.rellenaSeleccionDiasFormulario (miForm,
					listaPagaGuardias, listaNoPagaGuardias);
			...
			if (! hayHito45) hashResul.put ("45", numAsist);
			if (! hayHito46) hashResul.put ("46", numAct);
		...
		throws SIGAException
	{
		...
		try {
			...
			DefinirHitosFacturablesGuardiasForm miForm = 
					(DefinirHitosFacturablesGuardiasForm) formulario;
			Hashtable hash = new Hashtable ();
		...
		throws SIGAException
	{
		...
		try {
			...
			hash.put ("PAGOOFACTURACION", "F"); //cogerlo de un variable de session, pasada desde las pestanhas
	 ...
	 * Obtiene los datos del formulario para introducirlos en BD
	 * 
	 * @param  mapping - Mapeo de los struts
	 * @param  formulario -  Action Form asociado a este Action
	 * @param  request - objeto llamada HTTP 
		...
		throws SIGAException
	{
		...
		Hashtable originalHash = 
			(Hashtable) request.getSession ().getAttribute ("ORIGINALHASH");
		ScsHitoFacturableGuardiaAdm hitoAdm = 
			new ScsHitoFacturableGuardiaAdm (this.getUserBean (request));
		...
		try {
			...
			while (e.hasMoreElements ()) {
					...
					key.equals ("45") || key.equals ("46"))
				{
					...
					//sin embargo por pantalla se selecciona POR DIA, asi que es lo contrario 
				}
				if (key.equals ("5") || key.equals ("3") || key.equals ("10") ||
					key.equals ("7") || key.equals ("8") || key.equals ("19"))
				{
			...
			if (miForm.getChGuardias ())
			{
				...
				else if (miForm.getCheckB2 () && (miForm.getRadioA ().equals ("1"))) {
					hash.clear ();
					hash.put ("PAGOOFACTURACION", "F");
					hash.put ("IDINSTITUCION", usr.getLocation ());
					hash.put ("IDGUARDIA", originalHash.get ("IDGUARDIA"));
			...
			if (miForm.getChNoGuardias ())
			{
				...
				hash.put ("IDGUARDIA", originalHash.get ("IDGUARDIA"));
				hash.put ("IDTURNO", originalHash.get ("IDTURNO"));
				hash.put ("IDHITO", "25"); //importe por actuacion Fuera de Guardia
				hash.put ("PRECIOHITO", "0");
				hash.put ("PUNTOS", "0");
				hash.put ("FECHAMODIFICACION", "SYSDATE");
				hash.put ("USUMODIFICACION", usr.getUserName ());
				...
				if (miForm.getChActFG ()) {
					...
					hash.put ("IDGUARDIA", originalHash.get ("IDGUARDIA"));
					hash.put ("IDTURNO", originalHash.get ("IDTURNO"));
					hash.put ("IDHITO", "24"); //importe max. actuacion Fuera de Guardia
					hash.put ("PRECIOHITO", "0");
					hash.put ("PUNTOS", "0");
					hash.put ("FECHAMODIFICACION", "SYSDATE");
					hash.put ("USUMODIFICACION", usr.getUserName ());
		...
		throws SIGAException
	{
		...
		ScsGuardiasTurnoBean guardia = 
			(ScsGuardiasTurnoBean) request.getSession 
	...
	/**
	 * Obtiene la seleccion de dias que paga guardia
	 * desde el formulario que se muestra en pantalla
	 * 
	 * @param miForm
	 * @param listaPagaGuardia
	 * @param listaNoPagaGuardia
	...
	/**
	 * Obtiene la seleccion de dias que no paga guardia
	 * desde el formulario que se muestra en pantalla
	 * 
	 * @param miForm
	 * @param listaPagaGuardia
