			...
			HttpServletResponse response) throws SIGAException 
	{
		...
		try {
			...
			UsrBean user = (UsrBean) request.getSession().getAttribute("USRBEAN");
	     	...
	     	boolean isFichaColegial = action.equalsIgnoreCase("/JGR_ContrariosAsistenciaPerJGLetrado.do");
				...
				if (concepto.equals(PersonaJGAction.DESIGNACION_CONTRARIOS)) {
					...
					if (concepto.equals(PersonaJGAction.DESIGNACION_INTERESADO)) {
						...
						if (concepto.equals(PersonaJGAction.ASISTENCIA_CONTRARIOS)) {
							...
							if(isFichaColegial){
								ScsAsistenciasAdm scsAsistenciaAdm = new ScsAsistenciasAdm(user);
								...
								Hashtable<String, Object> asistenciaOriginalHashtable = scsAsistenciaAdm.getHashAsistenciaOriginalParaHistorico(asistenciaHashtable,false, user);
								
								if(!adm.deleteConHistorico(Long.valueOf((String)asistenciaOriginalHashtable.get("IDPERSONACOLEGIADO")), ht,null,campos,asistenciaHashtable))
					...
					if (concepto.equals(PersonaJGAction.EJG_UNIDADFAMILIAR)) {
						...
						if (v!=null && v.size()>0) {
							...
							if (b.getIdPersonaJG()!=null) {
								...
								if(personaBean.getIdPaisDireccion()!=null && (personaBean.getIdPaisDireccion().equalsIgnoreCase("")|| personaBean.getIdPaisDireccion().equalsIgnoreCase("191" ))) {
									miform.setIdPaisDireccion("191");
...
//										poblacionesBean.setNombre(nombrePoblacion);
									...
									miform.setIdPaisDireccion(personaBean.getIdPaisDireccion());
									miform.setDireccionExtranjera(personaBean.getDireccionExtranjera());	
								}
			...
			HttpServletResponse response) throws SIGAException 
	{
		try {
			...
			if (resultadoNIF!=null && resultadoNIF.size()>0) {
				...
				if(perBean.getIdPaisDireccion()!=null && (perBean.getIdPaisDireccion().equalsIgnoreCase("")|| perBean.getIdPaisDireccion().equalsIgnoreCase("191" ))) {
					...
					String direccionExtranjera  = perBean.getDireccionExtranjera();
					direccionExtranjera = direccionExtranjera.replaceAll("\n", " ");
					direccionExtranjera = direccionExtranjera.replaceAll("\r", " ");
					perBean.setDireccionExtranjera(direccionExtranjera);
					UtilidadesHash.setForCompare(hash,ScsPersonaJGBean.C_DIRECCIONEXTRANJERA,direccionExtranjera);
						UtilidadesHash.setForCompare(hash,ScsPersonaJGBean.C_IDPAISDIRECCION,perBean.getIdPaisDireccion());
				}	
		...
		try {
			...
			if (miform.getIdInstitucionJG()!=null && (miform.getIdPersonaJG()!=null && !miform.getIdPersonaJG().equals("null") && !miform.getIdPersonaJG().equals(""))) {
				...
				if (v!=null && v.size()>0) {
					...
					UtilidadesHash.setForCompare(hash,ScsPersonaJGBean.C_IDPAISDIRECCION,perBean.getIdPaisDireccion());
					UtilidadesHash.setForCompare(hash,ScsPersonaJGBean.C_DIRECCIONEXTRANJERA,perBean.getDireccionExtranjera());
    			...
    			//
					...
					miform.setPoblacion(perBean.getIdPoblacion());
					miform.setIdPaisDireccion(perBean.getIdPaisDireccion());
					miform.setDireccionExtranjera(perBean.getDireccionExtranjera());
			...
			HttpServletResponse response) throws SIGAException 
	{
		...
		try {
			...
			//mhg - INC_10319_SIGA
			if(ClsConstants.TIPO_SEXO_HOMBRE.equals(miform.getSexo()) || ClsConstants.TIPO_SEXO_MUJER.equals(miform.getSexo()) || ClsConstants.TIPO_SEXO_NC.equals(miform.getSexo())){
			...
			//Si el pais de la direccion es nulo o españa cogemos TODOS los datos de la direccio
			if(miform.getIdPaisDireccion()!=null && (miform.getIdPaisDireccion().equalsIgnoreCase("")|| miform.getIdPaisDireccion().equalsIgnoreCase("191" ))) {
				UtilidadesHash.set(persona,ScsPersonaJGBean.C_IDPAISDIRECCION,"191");
				UtilidadesHash.set(persona,ScsPersonaJGBean.C_DIRECCIONEXTRANJERA,"");
				...
				UtilidadesHash.set(persona,ScsPersonaJGBean.C_IDPAISDIRECCION,miform.getIdPaisDireccion());
				if(miform.getDireccionExtranjera()==null) {
					UtilidadesHash.set(persona,ScsPersonaJGBean.C_DIRECCIONEXTRANJERA,"");
					UtilidadesHash.set(persona,ScsPersonaJGBean.C_DIRECCION,"");
				}
				else {
					UtilidadesHash.set(persona,ScsPersonaJGBean.C_DIRECCIONEXTRANJERA,miform.getDireccionExtranjera());
					UtilidadesHash.set(persona,ScsPersonaJGBean.C_DIRECCION,miform.getDireccionExtranjera());
				}
				UtilidadesHash.set(persona,ScsPersonaJGBean.C_CODIGOPOSTAL,"");
				UtilidadesHash.set(persona,ScsPersonaJGBean.C_IDPROVINCIA,"");
				UtilidadesHash.set(persona,ScsPersonaJGBean.C_IDPOBLACION,"");
			...
			String numero="";
			String tipo="";
			boolean isFichaColegial = action.equalsIgnoreCase("/JGR_AsistidoAsistenciaLetrado.do") || action.equalsIgnoreCase("/JGR_ContrariosAsistenciaPerJGLetrado.do") ;
			...
			if (relacion.equalsIgnoreCase("guardarEJG")){
				...
				mapDestino = guardarRelacionAsistencia(mapping, form, request, response,isFichaColegial);
	...
	protected String guardarRelacionAsistencia (
			...
			MasterForm formulario, 
			HttpServletRequest request, 
			HttpServletResponse response,boolean isFichacolegial) throws SIGAException 
	{
			String result = "";
		try {
...
//			Hashtable persona= new Hashtable();
			...
			ScsAsistenciasBean beanASI = null;
			if (v!=null && v.size()>0) {
				beanASI= (ScsAsistenciasBean) v.get(0);
			}
			if (miform.getConceptoE().equals(PersonaJGAction.ASISTENCIA_ASISTIDO)) {
				// compruebo si ha cambiado el id persona para la relacion
			...
			if (miform.getConceptoE().equals(PersonaJGAction.ASISTENCIA_ASISTIDO)) {
				...
				if(isFichacolegial && isCambioAsistido){
					...
					admASI.updateDirecConHistoricoInsert(beanASI.getIdPersonaColegiado().longValue(),null,campos,asistenciaHashtable);
				}else{
			...
			if(miform.getConceptoE().equals(PersonaJGAction.ASISTENCIA_CONTRARIOS)) {
				...
				if (v2!=null && v2.size()>0) { 
					...
					if (miform.getAccionE().equals("editar")) {
						...
						if (old!=null) {
							if(isFichacolegial){
								...
								fksAsistenciaMap.put("Contrario",fksAsistenciaHashtable);
								asistenciaHashtable.put("fks", fksAsistenciaMap);
								if(!adm.deleteConHistorico(beanASI.getIdPersonaColegiado().longValue(), old,null,campos,asistenciaHashtable))
									throw new ClsExceptions("Error al borrar contrarios asistencia");
					...
					if(isFichacolegial){
						...
						fksAsistenciaMap.put("Contrario",fksAsistenciaHashtable);
						asistenciaHashtable.put("fks", fksAsistenciaMap);
						if(!adm.insertConHistorico(beanASI.getIdPersonaColegiado().longValue(), contrarioAsistenciaHash,null,campos,asistenciaHashtable))
							throw new ClsExceptions("Error al borrar contrarios asistencia");
