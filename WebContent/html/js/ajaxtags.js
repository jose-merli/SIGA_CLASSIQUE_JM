...
AjaxJspTag.Base = Class.create({
    ...
    initRequest: function () {
        if (Object.isFunction(this.options.onCreate)) {
            ...
            if ((Object.isString(result) && "cancel" === result.toLowerCase()) || (!Object.isUndefined(result) && !result)) {
    ...
    buildParameterString: function (ajaxParam) {
        ...
        params.split(',').each(function (pair) {
            ...
            if (!field) {
				...
				var radioGroup = field.name;
				var el = field.form;
				var checked=$(el).getInputs('radio', radioGroup).find(
					function(re) {return re.checked;}
				);
        		val.push($F(checked));
            } else if (['text', 'textarea', 'password', 'hidden', 'select-one'].include(field.type)) {
...
AjaxJspTag.HtmlContent = Class.create(AjaxJspTag.Base, {
    ...
    execute: function (event) {
        ...
        if (this.options.sourceClass && event) {
 ...
 * 
 */
AjaxJspTag.UpdateSelectFromField = Class.create(AjaxJspTag.Base, {
    initialize: function (options) {
        AjaxJspTag.add(this);
        this.setOptions(options);
        this.setListeners();
    ...
    handler: function () {
        ...
        this.parser.content.each(function (line) {
            ...
            if (line.length === 3){
            	newOption.selected = ("true" === line[2].toLowerCase()) || (this.defaultOptions.indexOf(line[1]) != -1);
            }
...
AjaxJspTag.UpdateFieldFromField = Class.create(AjaxJspTag.Base, {
    initialize: function (options) {
        AjaxJspTag.add(this);
        this.setOptions(options);
        this.setListeners();
...
AjaxJspTag.UpdateFieldFromSelect = Class.create(AjaxJspTag.Base, {
    initialize: function (options) {
        AjaxJspTag.add(this);
        this.setOptions(options);
        this.setListeners();
