...
<% 
	...
	Vector vDatosGDin = (Vector)request.getAttribute("datosGDin");
	request.removeAttribute("datosGDin");	
...
<html>
	...
	<script type="text/javascript" src="<html:rewrite page='/html/js/SIGA.js'/>"></script><script src="<html:rewrite page='/html/js/calendarJs.jsp'/>"></script>
		...
		<siga:TituloExt 
			titulo="facturacion.asignacionConceptos.gruposDinamicos.cabecera" 
			localizacion="facturacion.gruposDinamicos.literal.localizacion"/>
	...
	<body class="tablaCentralCampos">
		...
		<html:form action="/FAC_GruposDinamicos.do" method="POST" target="submitArea" style="display:none">
			...
			<%
			if (vDatosGDin==null || vDatosGDin.size()==0)
			{
			%>
					<siga:Table 
	   					name="tabladatos"
	   					border="1"
	   					columnNames="facturacion.gruposDinamicos.literal.gruposDeClientesDinamicos"
	   					columnSizes="100">
			<tr class="notFound">
			...
			else
			{
				...
				%>
					<siga:Table 
						...
						<%
						for (int i=0; i<vDatosGDin.size(); i++)
			   			{
							Hashtable miHash = (Hashtable)vDatosGDin.elementAt(i);
							%>
							...
							<tr class="listaNonEdit">
								<td>
									...
									<input type="hidden" name="oculto<%=""+(i+1)%>_3" value="<%=miHash.get("IDGRUPOSCRITERIOS")%>">
									<input type="hidden" name="oculto<%=""+(i+1)%>_4" value="<%=miHash.get("IDINSTITUCION_GRUP")%>">
									<%=miHash.get("NOMBREGRUPOCRITERIO")%>
								</td>
							</tr>
						<%}%>
