...
//
// Compara si las fechas pasadas como argumentos son iguales, o cual es mayor
// 0: Iguales
// 1: La primera es mayor
// 2: La segunda es mayor
// Devuelve un valor negativo: Error
//
function compararFecha (dato1, dato2)
{
	var devolucion = compararFechaRuano (dato1, dato2);
	if (devolucion == -2)
		return -1;
	else
		return devolucion;
...
//Funcion exacta a compararFecha, unicamente se difiere en que se devuelve un -2 si los dos campos de fecha son vacios
function compararFechaRuano (dato1, dato2) {
	String1 = "";
	String2 = "";
...
//
// Devuelve true: si la fecha pasada como argumento es invalida: sea futura o del dia actual
//
function esFechaNacimientoInvalida(valorFechaNacimiento) {
	if (valorFechaNacimiento=="") {
		return false; // no es invalida si esta vacia
	}
	
	var comparacionFechas = compararFecha(valorFechaNacimiento, getFechaActualDDMMYYYY());
	return (comparacionFechas < 2);
	// la fecha de nacimiento es invalida si es mayor que la fechaActual (1) o coinciden (0) 
}
...
function validarNig(nig,consejo){
	...
	if(consejo && consejo==IDINSTITUCION_CONSEJO_ANDALUZ){
		if(nig!=''){
			var objRegExp  = /^[0-9]{7}[S,C,P,O,I,V,M,6,8,1,2,3,4,0]{1}(19|20)\d{2}[0-9]{7}$/;
