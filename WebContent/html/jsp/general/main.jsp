...
<%
	...
	String interfaz = (String) request.getSession().getAttribute("InterfazInicio");
	String onLoad = "";
	if (interfaz!=null) {
		onLoad = "interfaz();";
	} else {
...
//				"width=750,height=200,scrollbars=no;resizable:no;top=100;left=100;Directories=no;Location=no;Menubar=no;Status=yes;Toolbar=no;"
			...
			function ayuda()
			{
				...
				window.open('<%=pathAyuda%>', 'Ayuda', 'width=800,height=600,scrollbars=no;resizable:no;top=100;left=100;Directories=no;Location=no;Menubar=no;Status=yes;Toolbar=no;');
				//Temporalmente abrimos una ventana con un mensaje de aviso en construccion
				//window.open('<%=app%>/html/jsp/general/enconstruccion.jsp', 'Ayuda', 'width=800,height=600,scrollbars=no;resizable:no;top=100;left=100;Directories=no;Location=no;Menubar=no;Status=yes;Toolbar=no;');
	...
	<body class="tableCabecera" onLoad="inicio();<%=onLoad%>;ajusteAlto('mainWorkArea');comprobarPops();" >
		...
		<div style="position:absolute; left:180; top:50px; z-index: 5;"> 
		...
		<iframe src="<%=app%>/html/jsp/general/entrada.jsp"
				id="mainWorkArea"
				name="mainWorkArea"
				frameborder="0"
				scrolling="no"
		...
		<div id="velo">
			<table class="tvelo"><tr><td><siga:Idioma key="messages.enProceso"/></td></tr></table>
		</div>
		
		<form name="formularioOcultoCerrarSesion" target="submitApplet" action="<%=app%>/html/jsp/general/cerrarSesion.jsp">
		</form>

    	<iframe name="frmAppletIP" src="<%=app%>/html/jsp/general/appletIP.jsp" style="display:none"></iframe>
