...
<%
	...
	try {
		...
		if (hash.containsKey("APELLIDO1ASISTIDO"))
			APELLIDO1ASISTIDO = hash.get("APELLIDO1ASISTIDO").toString();
		if (hash.containsKey("APELLIDO2ASISTIDO"))
			APELLIDO2ASISTIDO = hash.get("APELLIDO2ASISTIDO").toString();
		...
		if (hash.containsKey("FECHAAPERTURASOJ"))
			FECHAAPERTURASOJ = GstDate.getFormatedDateShort("", hash.get("FECHAAPERTURASOJ").toString());
		...
		if (hash.containsKey("DESIGNA_TURNO_NOMBRE"))
			DESIGNA_TURNO_NOMBRE = hash.get("DESIGNA_TURNO_NOMBRE").toString();
		...
		nombreCompleto = NOMBRELETRADO + " " + APELLIDO1LETRADO + " " + APELLIDO2LETRADO;
		TIPOEJGCOLEGIO = hash.get("TIPOEJGCOLEGIO").toString();
		if (hash.containsKey("FECHAPRESENTACION"))
			FECHAPRESENTACION = GstDate.getFormatedDateShort("", hash.get("FECHAPRESENTACION").toString());
		if (hash.containsKey("FECHALIMITEPRESENTACION") && (hash.get("FECHALIMITEPRESENTACION") != ""))
			FECHALIMITEPRESENTACION = GstDate.getFormatedDateShort("", hash.get("FECHALIMITEPRESENTACION").toString());
		...
		if (hash.containsKey("FECHARATIFICACION"))
			FECHARATIFICACION = hash.get("FECHARATIFICACION").toString();
		...
		if (hash.containsKey("IDINSTITUCION_PROC"))
			idInstitucionProcurador = hash.get("IDINSTITUCION_PROC").toString();
		...
		if ((String) request.getAttribute("PROCURADOR_NUM_COLEGIADO") != null)
			procuradorNumColegiado = (String) request.getAttribute("PROCURADOR_NUM_COLEGIADO");
		if ((String) request.getAttribute("PROCURADOR_NOMBRE_COMPLETO") != null)
			procuradorNombreCompleto = (String) request.getAttribute("PROCURADOR_NOMBRE_COMPLETO");
	} catch (Exception e) {
	}
	;
	...
	if (!(designaAnio == null || designaAnio.equals("")
		...
		Hashtable hastramitadordesigna = scsDesignaAdm.obtenerLetradoDesigna(usr.getLocation(), designaIdTurno, designaAnio, designaNumero);
	...
	String numeroDiligenciaAsi = (String) hash.get(ScsEJGBean.C_NUMERODILIGENCIA);
	String numeroProcedimientoAsi = (String) hash.get(ScsEJGBean.C_NUMEROPROCEDIMIENTO);
	String anioProcedimientoAsi = (String) hash.get(ScsEJGBean.C_ANIOPROCEDIMIENTO);	

	// Datos del Juzgado seleccionado:
	String juzgadoAsi = (String) hash.get(ScsEJGBean.C_JUZGADO);
	String juzgadoInstitucionAsi = (String) hash.get(ScsEJGBean.C_JUZGADOIDINSTITUCION);
	...
	String comisariaInstitucionAsi = (String) hash.get(ScsEJGBean.C_COMISARIAIDINSTITUCION);
	...
	Boolean tienePermisos = ((Boolean) request.getAttribute("tienePermisos")).booleanValue();
...
<body onload="cargarComboTipoColegio();">
								...
								<table width="100%" border="0"> 			
									<tr>	
										...
										<td class="labelTextValue" >
											<%=UtilidadesString.mostrarDatoJSP(DESIGNA_ANIO)%>
											/<%=UtilidadesString.mostrarDatoJSP(DESIGNA_CODIGO)%> 
											-<%=UtilidadesString.mostrarDatoJSP(t_nombreD)%> 
											<%=UtilidadesString.mostrarDatoJSP(t_apellido1D)%> 
											<%=UtilidadesString.mostrarDatoJSP(t_apellido2D)%>					
										</td>
	...
	<%
		...
		if ((modo != null) && ((modo.equalsIgnoreCase("consulta")) || (modo.equalsIgnoreCase("ver"))))
			condicion &= false;
