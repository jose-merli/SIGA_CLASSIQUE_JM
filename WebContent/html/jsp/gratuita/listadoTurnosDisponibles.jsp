...
<%
	...
	elems[0] = new FilaExtElement("solicitaralta", "solicitaralta",
...
<body>
	...
	<siga:TablaCabecerasFijas nombre="listarTurnosDisp" borde="1"
		clase="tableTitle" nombreCol="<%=nC%>" tamanoCol="<%=tC%>" alto="80%"
		ajusteBotonera="true" modal="G">

		<%
			String idPersona = (String) request
						.getAttribute("idPersonaTurno");
				if (obj.size() > 0) {
		%>
		<%
			int recordNumber = 1;
					while ((recordNumber) <= obj.size()) {
						...
						try {
							if (((String) hash.get("PARTIDAPRESUPUESTARIA"))
									.equals(""))
								o4 = "-1";
							else
						...
						if (sGuardias.equals("0")
								|| Integer.parseInt(nGuardiasTurno) == 0) {
							deshabilitarCheck = "";
						} else {
		...
		<siga:FilaConIconos fila='<%=String.valueOf(recordNumber)%>'
			...
			pintarEspacio="false">
			<input type='hidden' name="idPersonaSolicitud" id="idPersonaSolicitud"
				value='<%=idPersona%>'>
			<input type='hidden' name="idInstitucionSolicitud" id="idInstitucionSolicitud"
				value='<%=hash.get("IDINSTITUCION")%>'>
			<input type='hidden' id='oculto<%=String.valueOf(recordNumber)%>_1'
				name='oculto<%=String.valueOf(recordNumber)%>_1'
				value='<%=hash.get("IDTURNO")%>'>
			<input type='hidden' id='oculto<%=String.valueOf(recordNumber)%>_2'
				name='oculto<%=String.valueOf(recordNumber)%>_2'
				value='<%=hash.get("GUARDIAS")%>'>
			<input type='hidden' id='oculto<%=String.valueOf(recordNumber)%>_3'
				name='oculto<%=String.valueOf(recordNumber)%>_3'
				value='<%=hash.get("VALIDARJUSTIFICACIONES")%>'>
			<input type='hidden' id='oculto<%=String.valueOf(recordNumber)%>_4'
				name='oculto<%=String.valueOf(recordNumber)%>_4'
				value='<%=hash.get("VALIDARINSCRIPCIONES")%>'>
			<input type='hidden' id='oculto<%=String.valueOf(recordNumber)%>_5'
				name='oculto<%=String.valueOf(recordNumber)%>_5'
				value='<%=hash.get("DESIGNADIRECTA")%>'>
			<input type='hidden' id='oculto<%=String.valueOf(recordNumber)%>_6'
				name='oculto<%=String.valueOf(recordNumber)%>_6'
				value='<%=hash.get("REPARTOPORPUNTOS")%>'>
			<input type='hidden' id='oculto<%=String.valueOf(recordNumber)%>_7'
				name='oculto<%=String.valueOf(recordNumber)%>_7' value='<%=o1%>'>
			<input type='hidden' id='oculto<%=String.valueOf(recordNumber)%>_8'
				name='oculto<%=String.valueOf(recordNumber)%>_8' value='<%=o2%>'>
			<input type='hidden' id='oculto<%=String.valueOf(recordNumber)%>_9'
				name='oculto<%=String.valueOf(recordNumber)%>_9'
				value='<%=hash.get("IDORDENACIONCOLAS")%>'>
			<input type='hidden' id='oculto<%=String.valueOf(recordNumber)%>_10'
				name='oculto<%=String.valueOf(recordNumber)%>_10' value='<%=o5%>'>
			<input type='hidden' id='oculto<%=String.valueOf(recordNumber)%>_11'
				name='oculto<%=String.valueOf(recordNumber)%>_11'
				value='<%=hash.get("IDAREA")%>'>
			<input type='hidden' id='oculto<%=String.valueOf(recordNumber)%>_12'
				name='oculto<%=String.valueOf(recordNumber)%>_12'
				value='<%=hash.get("IDMATERIA")%>'>
			<input type='hidden' id='oculto<%=String.valueOf(recordNumber)%>_13'
				name='oculto<%=String.valueOf(recordNumber)%>_13'
				value='<%=hash.get("IDZONA")%>'>
			<input type='hidden' id='oculto<%=String.valueOf(recordNumber)%>_14'
				name='oculto<%=String.valueOf(recordNumber)%>_14' value='<%=o3%>'>
			<input type='hidden' id='oculto<%=String.valueOf(recordNumber)%>_15'
				name='oculto<%=String.valueOf(recordNumber)%>_15' value='<%=o4%>'>
			<input type='hidden' id='oculto<%=String.valueOf(recordNumber)%>_16'
				name='oculto<%=String.valueOf(recordNumber)%>_16' value='<%=o5%>'>
			<input type='hidden' id='oculto<%=String.valueOf(recordNumber)%>_17'
				name='oculto<%=String.valueOf(recordNumber)%>_17'
				value='<%=hash.get("IDGRUPOFACTURACION")%>'>

			<%
				// Para posibles valores nulos.
								String partidaPresupuestaria = " ";
								if (hash.get("PARTIDAPRESUPUESTARIA") != null
										&& !hash.get("PARTIDAPRESUPUESTARIA")
												.equals(""))
									partidaPresupuestaria = (String) hash
											.get("PARTIDAPRESUPUESTARIA");
			%>
			<input type='hidden' id='oculto<%=String.valueOf(recordNumber)%>_18'
				name='oculto<%=String.valueOf(recordNumber)%>_18'
				value='<%=partidaPresupuestaria%>'>
			<input type='hidden' id='oculto<%=String.valueOf(recordNumber)%>_19'
				name='oculto<%=String.valueOf(recordNumber)%>_19'
				value='<%=hash.get("GRUPOFACTURACION")%>'>
			<input type='hidden' id='oculto<%=String.valueOf(recordNumber)%>_20'
				name='oculto<%=String.valueOf(recordNumber)%>_20' value='<%=o6%>'>
			<td align="center"><input type="checkBox" name="altaTurno"
				id="<%="altaTurno"
									+ String.valueOf(recordNumber)%>"
				<%=deshabilitarCheck%>></td>
	...
	<siga:ConjBotonesAccion botones="altaEnTurnosSel,X"
