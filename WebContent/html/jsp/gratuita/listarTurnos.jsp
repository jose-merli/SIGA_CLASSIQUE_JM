...
<html>
	<head>
		...
		if (!isEntradaSJCS){ %>
			<siga:TituloExt titulo="censo.fichaCliente.sjcs.turnos.cabecera" localizacion="censo.fichaCliente.sjcs.turnos.localizacion"/>
		<% } %>
	...
	<body class="tablaCentralCampos" onload="mostrarFecha();">    
		   ...
		   activarFilaSel="<%=activaSeleccionFila%>">
			...
			<%} else {		
				for (int j = 0; j < resultado.size(); j++) {
					...
					try{
						if(((String)registro.get("DESCRIPCION")).equals(""))
							o1="-";
						else 
							o1=(String)registro.get("DESCRIPCION");
					} catch(Exception e){
						o1="-";
					}
					
					try {
						if(((String)registro.get("REQUISITOS")).equals(""))
							o2="-";
						else 
							o2=(String)registro.get("REQUISITOS");
					} catch(Exception e){
						o2="-";
					}
					
					try {
						if(((String)registro.get("IDSUBZONA")).equals(""))
							o3="-1";
						else 
							o3=(String)registro.get("IDSUBZONA");
					} catch(Exception e){
						o3="-1";
					}
					
					try {
						if(((String)registro.get("IDPARTIDAPRESUPUESTARIA")).equals(""))
							o4="-1";
						else 
							o4=(String)registro.get("IDPARTIDAPRESUPUESTARIA");
					} catch(Exception e){
						o4="-1";
					}
					
					try {
						if(((String)registro.get("IDPERSONAULTIMO")).equals(""))
							o5="-1";
						else 
							o5=(String)registro.get("IDPERSONAULTIMO");
					} catch(Exception e){
						o5="-1";
					}
					...
					<siga:FilaConIconos fila='<%=String.valueOf(i)%>' botones="<%=botones%>" visibleBorrado="<%=String.valueOf(isEntradaSJCS)%>" visibleEdicion="<%=String.valueOf(isEntradaSJCS)%>" pintarEspacio="<%=String.valueOf(isEntradaSJCS)%>" elementos='<%=elems%>' clase="listaNonEdit">				
						...
						<%if (!isEntradaSJCS){%>
							...
							<td>
								<%if (!((String)registro.get("SUBZONA")).equals("")){%>
									&nbsp;<%=registro.get("SUBZONA")%>
								<%} else {%> 
									&nbsp; 
								<%}%>						
							</td>
		...
		<script>		
			...
			function buscar() {
				document.FormASolicitar.action		 = "/SIGA/JGR_DefinirTurnosLetradoAction.do?granotm=<%=System.currentTimeMillis()%>";
			...
			function refrescarLocal(incluirBajaLogica) {			
				document.DefinirTurnosLetradoForm.action = "/SIGA/JGR_DefinirTurnosLetrado.do";
