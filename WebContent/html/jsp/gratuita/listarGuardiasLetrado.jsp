...
<siga:TituloExt titulo="censo.fichaCliente.sjcs.guardias.cabecera"
...
<body class="tablaCentralCampos" onload="mostrarFecha();">
	<bean:define id="bIncluirBajaLogica" property="bajaLogica"
		name="DefinirGuardiasLetradoForm"></bean:define>
	<%
	...
	<table class="tablaTitulo" cellspacing="0">
		<tr>
			<td class="titulitosDatos"><siga:Idioma
					key="censo.fichaCliente.guardiasInscrito.pestana.titulito" />&nbsp;&nbsp;<%=UtilidadesString.mostrarDatoJSP(nombrePestanha)%>&nbsp;&nbsp;
				<% if(numeroPestanha!= null && !numeroPestanha.equalsIgnoreCase("")) { %>
				<%if (estadoColegial!=null && !estadoColegial.equals("")){%> <siga:Idioma
					key="censo.fichaCliente.literal.colegiado" /> <%= UtilidadesString.mostrarDatoJSP(numeroPestanha)  %>
			...
			<table>
				<tr>
					<td class="labelText"><siga:Idioma
							key="gratuita.listarGuardiasLetrado.literal.textoInscripcion" />
					</td>
				</tr>
			</table>
		</div>
	...
	<siga:TablaCabecerasFijas nombre="tablaDatos" borde="1"
		...
		tamanoCol="15,15,10,8,6,8,8,8,8,7,8" alto="100%" ajuste="50"
		ajusteBotonera="true">
		<% if (obj.size()>0) { 
	    	int recordNumber=1;
		...
		<siga:FilaConIconos fila='<%=String.valueOf(recordNumber)%>'
			...
			<td>
				<%if(((String)hash.get("OBLIGATORIEDAD")).equalsIgnoreCase("0")){%>Obligatorias<%}else if(((String)hash.get("OBLIGATORIEDAD")).equalsIgnoreCase("1")){%>Todas
				o Ninguna<%}else{%>A elección<%}%>
			</td>
			<td><%=ScsGuardiasTurnoAdm.obtenerTipoDia((String)hash.get("SELECCIONLABORABLES"), (String)hash.get("SELECCIONFESTIVOS"), usr)%>
			</td>
			<td><%=hash.get("DURACION")%>&nbsp;<siga:Idioma
					key="<%=literalDuracion%>" /></td>
			<td>&nbsp;<%=GstDate.getFormatedDateShort("",fechaSolicitud)%><siga:Idioma
					key="<%=literalValidar%>" /></td>
	...
	<div style="position: absolute; left: 400px; bottom: 25px; z-index: 2;">
		<table align="center" border="0">
			<tr>
				<td class="labelText"><siga:Idioma
						key="gratuita.gestionInscripciones.vertodas" /> <% if (bIncluirBajaLogica.equals("S")) { %>
	...
	<html:form action="/JGR_ListarGuardiasLetrado.do" method="post"
		target="">
		<input type="hidden" name="actionModal" value="" />
		<html:hidden name="DefinirGuardiasLetradoForm" property="bajaLogica" />
		<html:hidden name="DefinirGuardiasLetradoForm"
			property="fechaConsulta" />
		<html:hidden name="DefinirGuardiasLetradoForm" property="modo" />
	</html:form>

	<html:form action="/JGR_BajaTurnos" name="FormAConsultar" styleId="FormAConsultar"
