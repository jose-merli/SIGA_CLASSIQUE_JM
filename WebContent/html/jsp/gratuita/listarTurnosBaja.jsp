...
<script>
	...
	function validar(fila) 
	{
		   ...
		   document.forms[0].action							= "<%=app%>/JGR_BajaTurnos.do";
   		   ...
   		   document.forms[0].confirmacion.value 			= "0";
		...
		function preguntaConfirmacion(mensajeConfirmacion)
		{
			if (mensajeConfirmacion != null && mensajeConfirmacion != "") {
				if(confirm(mensajeConfirmacion)) {	
					  document.forms[0].confirmacion.value = "1";
					  var resultado = ventaModalGeneral(document.forms[0].name, "G");
		...
		<%if (obj != null && obj.size()>0){%>
				...
				%>
					<siga:FilaConIconos fila='<%=String.valueOf(recordNumber)%>' botones="<%=botones%>" elementos='<%=elems%>' clase="listaNonEdit" visibleConsulta="no" visibleEdicion="no" visibleBorrado="no">
						...
						<td  align="center"><%=UtilidadesString.mostrarDatoJSP(hash.get("NOMBRE_PERSONA"))%> <%=UtilidadesString.mostrarDatoJSP(hash.get("APELLIDO1_PERSONA"))%> <%=UtilidadesString.mostrarDatoJSP(hash.get("APELLIDO2_PERSONA"))%></td>
