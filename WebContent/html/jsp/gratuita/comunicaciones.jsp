...
<table class="tablaTitulo" cellspacing="0" heigth="38">
	<tr>
		<td class="titulitosDatos">
			<c:out value="${ComunicacionesForm.anio}"/>/<c:out
			 	value="${ComunicacionesForm.codigoDesignaNumEJG}"/>&nbsp;-&nbsp;<c:out 
			 	value="${ComunicacionesForm.solicitante}"/>
		</td>
	</tr>
</table>
...
<siga:Table	 name="comunicacionesSalida"
		   		  border="1"
	...
	<c:choose>
		<c:when test="${empty comunicacionesSalida}">
			<tr class="notFound">
	   			<td colspan = "6" class="titulitos"><siga:Idioma key="messages.noRecordFound"/></td>
			</tr>
			
		</c:when>
		<c:otherwise>
				<c:forEach items="${comunicacionesSalida}" var="comunicacionSalida" varStatus="status">
				<siga:FilaConIconosExtExt fila='${status.count}' botones="${comunicacionSalida.botones}" elementos="${comunicacionSalida.elementosFila}" 
				nombreTablaPadre="comunicacionesSalida"  clase="listaNonEdit">
					...
					<input type="hidden" id="idIntercambio_${status.count}" value = "${comunicacionSalida.idIntercambio}"/>
						...
						<td align="center">
							<c:out value="${comunicacionSalida.fecha}"/>
						</td>
						<td align="center"><c:out value="${comunicacionSalida.fechaProgramada}"/>
						</td>
						<td ><siga:Idioma key="${comunicacionSalida.estado}"></siga:Idioma>
						</td>
						<td><siga:Idioma key="${comunicacionSalida.tipoEnvio}"></siga:Idioma></td>
