...
<% 
	...
	String idCampoCodigoRel ="";
	String descripcionRel ="";
	String querycombo ="";
	...
	<body>
		<html:form action="/ADM_GestionarTablasMaestras.do" method="POST" target="submitArea">
			...
			<%if(sNombreTabla.equals("SCS_COSTEFIJO")){ %> 
 			<div id="divListadoActuaciones" style='height: 100%; position: absolute; width: 100%; overflow-y: auto'> 
					<siga:Table 
						name="tablaDatos" 
						border="1" 
						columnNames="administracion.catalogos.maestros.literal.tipos.asistencias,administracion.catalogos.costesFijos.importe," 
						columnSizes="75,15,10">
					<c:choose>
							...
							<c:forEach items="${tiposAsistenciasRel}" var="tipoAsistencia" varStatus="status">															
								<siga:FilaConIconos	fila='${status.count}'				    
						  			...
						  			clase="listaNonEdit">
										...
										<td align='right'>
											<c:out	value="${tipoAsistencia.IMPORTE}" />
										</td>
								</siga:FilaConIconos>
							</c:forEach>
						</c:otherwise>
