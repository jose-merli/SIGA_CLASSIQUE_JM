...
<% 
	...
	if (modo.equals("ver") || modo.equals("editar")) {
		...
		if (htData != null) {
			...
			if ((aux != null) && (aux.equals("true")))	preferenteFax = true;
			aux = (String)request.getAttribute("preferenteMail");
			if ((aux != null) && (aux.equals("true")))	preferenteMail = true;
			aux = (String)request.getAttribute("preferenteCorreo");
			if ((aux != null) && (aux.equals("true")))	preferenteCorreo = true;
			aux = (String)request.getAttribute("preferenteSms");
			if ((aux != null) && (aux.equals("true")))	preferenteSms = true;
			...
			if (modo.equals("editar")) {
				...
				ididPais=(htData.get(CenDireccionesBean.C_IDPAIS)==null || String.valueOf(htData.get(CenDireccionesBean.C_IDPAIS)).equals(""))?ClsConstants.ID_PAIS_ESPANA:String.valueOf(htData.get(CenDireccionesBean.C_IDPAIS));
				idPais.add(ididPais);
...
<head>
	...
	<html:javascript formName="consultaDireccionesForm" staticJavascript="false" />  
	...
	<table class="tablaTitulo" cellspacing="0" heigth="32">
		<tr>
			<td id="titulo" class="titulosPeq">
				<siga:Idioma key="censo.consultaDirecciones.literal.titulo1"/> &nbsp;&nbsp;<%=UtilidadesString.mostrarDatoJSP(nombreUsu)%> &nbsp;&nbsp;
	...
	<!-- Zona de campos de busqueda o filtro -->
  	...
  	<html:form action="/CEN_ConsultasDirecciones" method="POST" target="resultado">
		...
		<%if (editarCampos) {%>
			...
			<html:hidden property="idTipoDireccionAntes" 	value="<%=idTipoDireccion%>"/> 
							...
							<tr>
								...
								<%	
								 ...
								 %>   		
						          ...
						          <input type=checkbox id="checkTipoDireccion" value="<%=idTipoDireccion1%>" <%=activarCheck%> onclick="comprobarTelefonoAsterico()" <%=desactivarCheckTipos%>/>
...
<br>
								...
								<td valign="top">														
								  <table>
					   			    <tr>
						   		      <td class="labelText"> 
						   		        <p align="right">
						   		          <siga:Idioma key="censo.datosDireccion.literal.fechaModificacion"/>&nbsp&nbsp<%=fechaModificacion%>
						   		        </p>
						   		      </td>
					   			    </tr>
					   			    <% if (!fechaBaja.equals("")) { %>
							...
							<tr>		
								<td class="labelText" width="180px" id="direccionSinAsterisco"><siga:Idioma key="censo.datosDireccion.literal.direccion"/>&nbsp</td>
								<td class="ocultar" width="180px" id="direccionConAsterisco"><siga:Idioma key="censo.datosDireccion.literal.direccion"/>&nbsp(*)</td>
								<td><html:textarea cols="70"  rows="2" name="consultaDireccionesForm" property="domicilio" onKeyDown="cuenta(this,100)" onChange="cuenta(this,100)" value="<%=domicilio%>"  styleClass="<%=clase%>" readOnly="<%=desactivado%>" ></html:textarea></td>
							...
							<tr>		
								<td class="labelText" width="180px" id="paisSinAsterisco"><siga:Idioma key="censo.datosDireccion.literal.pais2"/>&nbsp</td>
								<td class="ocultar" width="180px" id="paisConAsterisco"><siga:Idioma key="censo.datosDireccion.literal.pais2"/>&nbsp</td>
								<td>
		   						<%if (editarCampos){%>
									<siga:ComboBD nombre="pais" tipo="pais" clase="boxCombo" obligatorio="false" elementoSel="<%=idPais%>" accion="selPais(this.value);"/>
								<%} else {%>
									<html:hidden property="pais" value='<%=ididPais%>' ></html:hidden>
									<html:text name="consultaDireccionesForm" property="pais2" value='<%=pais%>' size="40" styleClass="<%=clase%>" readOnly="<%=desactivado%>"></html:text>
								<%}%>
								</td>
							...
							<tr>
								<td class="labelText" id="provinciaSinAsterisco"><siga:Idioma key="censo.datosDireccion.literal.provincia"/>&nbsp</td>
								<td class="ocultar" id="provinciaConAsterisco"><siga:Idioma key="censo.datosDireccion.literal.provincia"/>&nbsp(*)</td>
								<td id="provinciaEspanola">
		   						<%if (editarCampos){%>
									...
									<%} else {
									%>
										<html:text property="provincia" value="<%=provincia%>" size="40" styleClass="<%=clase%>" readOnly="<%=desactivado%>"></html:text>
									<%}%>
								</td>

								<td class="labelText" id="poblacionSinAsterisco"><siga:Idioma key="censo.datosDireccion.literal.poblacion"/>&nbsp</td>
								<td class="ocultar" id="poblacionConAsterisco"><siga:Idioma key="censo.datosDireccion.literal.poblacion"/>&nbsp(*)</td>
								<td id="poblacionEspanola">
		   						<%if (editarCampos){%>
									...
									%>
										...
										<html:text property="poblacion" value="<%=poblacion%>" size="40" styleClass="<%=clase%>" readOnly="<%=desactivado%>"></html:text>
									<%}%>
								</td>
								<td  class="ocultar"  id="poblacionExtranjera">
										<html:text name="consultaDireccionesForm" property="poblacionExt" value='<%=poblacionExt%>' size="30" styleClass="<%=clase%>" readOnly="<%=desactivado%>"></html:text>
								</td>
		  				</tr>

						<tr>
		   					<td class="labelText" id="telefonoSinAsterisco"><siga:Idioma key="censo.datosDireccion.literal.telefono1"/>&nbsp</td>
		   					<td class="ocultar"   id="telefonoConAsterisco"><siga:Idioma key="censo.datosDireccion.literal.telefono1"/>&nbsp(*)</td>
		   					<td><html:text name="consultaDireccionesForm" property="telefono1" value="<%=telefono1%>" maxlength="20" size="10" styleClass="<%=clase%>" readOnly="<%=desactivado%>" ></html:text></td>

		   					<td class="labelText"><siga:Idioma key="censo.datosDireccion.literal.telefono2"/>&nbsp</td>				
		   					<td><html:text name="consultaDireccionesForm" property="telefono2" value="<%=telefono2%>" maxlength="20" size="10" styleClass="<%=clase%>" readOnly="<%=desactivado%>" ></html:text></td>	
		 				</tr>

		 				<tr>
		   					<td class="labelText"><siga:Idioma key="censo.datosDireccion.literal.movil"/>&nbsp</td>				
		   					<td><html:text name="consultaDireccionesForm" property="movil" value="<%=movil%>" maxlength="20" size="10" styleClass="<%=clase%>" readOnly="<%=desactivado%>"></html:text></td>	
		 				</tr>

		 				<tr>
		   					<td class="labelText"><siga:Idioma key="censo.datosDireccion.literal.fax1"/>&nbsp</td>
		   					<td><html:text name="consultaDireccionesForm" property="fax1" value="<%=fax1%>" maxlength="20" size="10" styleClass="<%=clase%>" readOnly="<%=desactivado%>"></html:text></td>
		  				 	
		  				 	<td class="labelText"><siga:Idioma key="censo.datosDireccion.literal.fax2"/>&nbsp</td>				
		   					<td><html:text name="consultaDireccionesForm" property="fax2" value="<%=fax2%>" maxlength="20" size="10" styleClass="<%=clase%>" readOnly="<%=desactivado%>"></html:text></td>
		  				</tr>
						  ...
						  <tr>
						  	...
						  	<td colspan="2" class="labelText">
						  		  	<siga:Idioma key="censo.preferente.mail"/><input type="checkbox" name="preferenteMail"   <%if (desactivado) out.print("disabled");%>  <%if (preferenteMail)   out.print(" checked");%>>&nbsp;&nbsp;&nbsp;
					   				<siga:Idioma key="censo.preferente.correo"/><input type="checkbox" name="preferenteCorreo" <%if (desactivado) out.print("disabled");%>  <%if (preferenteCorreo) out.print(" checked");%>>&nbsp;&nbsp;&nbsp;
					   				<siga:Idioma key="censo.preferente.fax"/><input type="checkbox" name="preferenteFax"    <%if (desactivado) out.print("disabled");%>  <%if (preferenteFax)    out.print(" checked");%>>&nbsp;&nbsp;&nbsp;  	
					   				<siga:Idioma key="censo.preferente.sms"/><input type="checkbox" name="preferenteSms"    <%if (desactivado) out.print("disabled");%>  <%if (preferenteSms)    out.print(" checked");%>>
					   		</td>
		  				</tr>
		   			</table>
