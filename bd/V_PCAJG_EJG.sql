...
SELECT EJGREMESA.IDREMESA
      ...
      , (CASE
       ...
       , 'SG##SIGA' AS ORIGENSISTEMA_CDA
       , (CASE
               WHEN I.CEN_INST_IDINSTITUCION = 3004
               THEN 'AJG'
               ELSE 'GEN'
      ...
      --Datos Expedientes
      /*, (SELECT P.CODIGO
                FROM PCAJG_ICAS P, PCAJG_ICAS_CENINSTITUCION PS
                WHERE P.IDENTIFICADOR = PS.IDENTIFICADOR
                AND P.IDINSTITUCION = PS.IDINSTITUCION_PCAJG
                AND PS.IDINSTITUCION_SIGA = EJGREMESA.IDINSTITUCIONREMESA
                AND PS.IDINSTITUCION_PCAJG = EJG.IDINSTITUCION) AS DE_CE_COLEGIOEXPEDIENTE --CAMBIADO POR LP*/
      /*, (SELECT T.CODIGOEXT FROM SCS_TIPOEJGCOLEGIO T
                 WHERE T.IDINSTITUCION = EJG.IDINSTITUCION
                 AND T.IDTIPOEJGCOLEGIO = EJG.IDTIPOEJGCOLEGIO) AS DE_CE_COLEGIOEXPEDIENTE --confirmado por asoler */ --TODO ÑAPA PARA LOS EXPEDIENTES NUEVOS CON UN SOLO TIPOEJGCOLEGIO DESCOMENTAR ESTO
      ...
      , (CASE WHEN VPROC.IDPROCURADOR IS NOT NULL
              THEN (SELECT P.CODIGO FROM PCAJG_ICAPROCURADOR P, PCAJG_ICAPROC_CENINSTI PS
                                    WHERE P.IDENTIFICADOR = PS.IDENTIFICADOR
                                    AND P.IDINSTITUCION = PS.IDINSTITUCION_PCAJG
                                    AND PS.IDINSTITUCION_SIGA = VPROC.IDINSTITUCION
                                    AND P.IDINSTITUCION = EJGREMESA.IDINSTITUCION)
              WHEN PROC.IDPROCURADOR IS NOT NULL
              THEN (SELECT P.CODIGO FROM PCAJG_ICAPROCURADOR P, PCAJG_ICAPROC_CENINSTI PS
                                    WHERE P.IDENTIFICADOR = PS.IDENTIFICADOR
                                    AND P.IDINSTITUCION = PS.IDINSTITUCION_PCAJG
                                    AND PS.IDINSTITUCION_SIGA = PROC.IDINSTITUCION
                                    AND P.IDINSTITUCION = EJGREMESA.IDINSTITUCION)
              ELSE NULL
      ...
      , VDP.EMAIL AS DS_DC_EMAIL
          ...
          , (CASE WHEN UF.DESCRIPCIONINGRESOSANUALES IS NOT NULL
      ...
      , (CASE
              WHEN
                D.IDJUZGADO IS NOT NULL
              THEN
                (SELECT DECODE(I.CEN_INST_IDINSTITUCION, 3003, LPAD(PJ.IDPARTIDO, 4, '0'), PJ.CODIGOEXT) || '##' || PJ.NOMBRE
              ...
              THEN
                ...
                AND POB.IDPARTIDO = PJ.IDPARTIDO)
			  WHEN
				I.CEN_INST_IDINSTITUCION IN (3001, 3010)
              THEN
                DECODE(LEN.IDLENGUAJE, 2, '-1##Indeterminat', '-1##Indeterminado')
			  ELSE
				NULL
              END) AS DDJ_PARTIDOJUDICIAL_CDA
      /*, NVL((SELECT DECODE(J.IDJURISDICCION, NULL, NULL, J.CODIGOEXT || '##' || F_SIGA_GETRECURSO(J.DESCRIPCION, LEN.IDLENGUAJE))
            FROM SCS_JURISDICCION J, SCS_PROCEDIMIENTOS P, SCS_DESIGNA D, SCS_EJGDESIGNA ED
            WHERE J.IDJURISDICCION = P.IDJURISDICCION
            AND P.IDINSTITUCION = D.IDINSTITUCION
            AND P.IDPROCEDIMIENTO = D.IDPROCEDIMIENTO
            ...
            AND ED.ANIODESIGNA || LPAD(ED.NUMERODESIGNA, 10, '0') = --si hay mas de una designa cogemos la primera (LP)
                  (SELECT MIN(ED2.ANIODESIGNA || LPAD(ED2.NUMERODESIGNA, 10, '0'))
                  FROM SCS_EJGDESIGNA ED2
                  WHERE ED2.IDINSTITUCION = EJG.IDINSTITUCION
                  AND ED2.ANIOEJG = EJG.ANIO
                  AND ED2.NUMEROEJG = EJG.NUMERO
                  AND ED2.IDTIPOEJG = EJG.IDTIPOEJG)), '-1##Indeterminado') AS DDJ_JURISDICCION_CDA
      , DECODE(D.NUMERO, NULL, '-1##Indeterminado',
               (SELECT DECODE(P.IDENTIFICADOR, NULL, '-1##Indeterminado', P.CODIGO || '##' || P.DESCRIPCION || '##' || P.ABREVIATURA)
                FROM PCAJG_AREA P, PCAJG_AREA_SCSAREA PS, SCS_TURNO T
                WHERE D.IDINSTITUCION = T.IDINSTITUCION
                AND D.IDTURNO = T.IDTURNO
                AND T.IDINSTITUCION = PS.IDINSTITUCION
                AND T.IDAREA = PS.IDAREA
                AND PS.IDINSTITUCION = P.IDINSTITUCION
                AND PS.IDENTIFICADOR = P.IDENTIFICADOR)) AS DDJ_JURISDICCION_CDA*/
      , (CASE
      ...
      , (CASE
              /*WHEN (((EJGREMESA.IDINSTITUCION || EJGREMESA.IDREMESA) = 2047159) AND VASIS.IDPRETENSION IS NOT NULL)--ÑAPA
              THEN (SELECT PRE_A.CODIGOEXT || '##' || SUBSTR(F_SIGA_GETRECURSO(PRE_A.DESCRIPCION, LEN.IDLENGUAJE), 1, 50) FROM SCS_PRETENSION PRE_A
              WHERE PRE_A.IDINSTITUCION = VASIS.IDINSTITUCION AND PRE_A.IDPRETENSION = VASIS.IDPRETENSION)*/
...
--              WHEN (NVL(PRE_D.CODIGOEXT, PRE.CODIGOEXT) IN ('2101501000','2101502000', '0') OR (EJGREMESA.IDINSTITUCION || EJGREMESA.IDREMESA) = 2047159)--Atestat, Diligència, Altres
      ...
      --Datos Tramitacion Expediente
        ...
        , DECODE(EJG.FECHADICTAMEN, NULL, NULL, DECODE(LEN.IDLENGUAJE, 2, '3##Dictaminat', '3##Dictaminado')) AS DTE_TD_IT_ESTADOEXPEDIENTE_CDA
