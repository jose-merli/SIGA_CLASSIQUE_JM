...
PROCEDURE PROC_FCS_MARCAR_FACTURADOS(
  ...
  ****************************************************************************************************************/
    FUNCTION FUNC_CALCULAR_IMPORTEASIST(
        ...
        P_IDTURNO IN SCS_ASISTENCIA.IDTURNO%TYPE,
        P_IDGUARDIA IN SCS_ASISTENCIA.IDGUARDIA%TYPE,
        P_IDTIPOASISTENCIA IN SCS_ASISTENCIA.IDTIPOASISTENCIACOLEGIO%TYPE,
        P_FECHA IN SCS_ASISTENCIA.FECHAHORA%TYPE,
...
CREATE OR REPLACE PACKAGE BODY PKG_SIGA_FACTURACION_SJCS IS
    ...
    c_Id_Soria       Constant Scs_Asistencia.Idinstitucion%Type := 2067;
    ...
    */
    CURSOR C_ASISTENCIAS_UG_CONTROLADO(
        ...
        SELECT ASI.IDINSTITUCION,
            ...
            DECODE(V_APLICATIPOS, 0, NULL, FUNC_CALCULAR_IMPORTEASIST(ASI.IDINSTITUCION, ASI.IDTURNO, ASI.IDGUARDIA, ASI.IDTIPOASISTENCIACOLEGIO, V_FECHADIA, NULL,0)) AS IMPORTETIPO -- Obtiene el importe del tipo de la asistencia, en el caso de aplicar tipos
          FROM SCS_ASISTENCIA ASI
    ...
    BEGIN
        ...
        IF (V_IDFACTURACION IS NOT NULL) THEN
            BEGIN
                ...
                FROM FCS_HISTORICO_TIPOASISTCOLEGIO HTASI
  ...
  ********************************************************/
  ...
  -- ------------   ----------------------   -------------------------------------------------------------------
  --  14/03/2005    Raul Gonzalez Gonzalez   Creacion.
  --  07/03/2006    Pilar Duran              Se obtiene ademas del precio total el valor del Punto.
  --  01/10/2009    Juan antonio Saiz        Se elimina la facturacion por puntos.
  --  15/02/2017    Adrian Ayala Gomez       Para el caso especial de Alcala, se factura cada actuacion de Fin
  --                                         a partir del 100% del modulo, restando lo facturado
  --                                         por la actuacion de Inicio de la misma designacion y modulo.
  --
  Procedure Proc_Fcs_Facturar_Turnos_Ofi(p_Idinstitucion   In Number,
                                         p_Idfacturacion   In Number,
                                         p_Usumodificacion In Number,
    ...
    v_Fechadesde Date;
    v_Fechahasta Date;
    v_Total      Number; -- acumulador para el total de oficio
    v_Precio     Number; -- importe a facturar por cada actuacion
    v_Porcentaje Number; -- porcentaje a aplicar al importe que se factura a cada actuacion
    ...
    Cursor c_Gruposfacturacion Is
      ...
      , Fcs_Fact_Grupofact_Hito.factconvenio
       ...
       Where Fcs_Fact_Grupofact_Hito.Idinstitucion = p_Idinstitucion
         ...
         And Fcs_Fact_Grupofact_Hito.Idhitogeneral = Pkg_Siga_Constantes.Hito_General_Turno;
    ...
    Cursor c_Actdesigna(p_Idturno Number, p_Fechadesde Date, p_Fechahasta Date, p_factconvenio Number) Is
      Select Scs_Actuaciondesigna.Idinstitucion,
       ...
       Where Scs_Actuaciondesigna.Idinstitucion = Scs_Designa.Idinstitucion
         ...
         And Scs_Actuaciondesigna.Anio = Scs_Designa.Anio
         And Scs_Actuaciondesigna.Numero = Scs_Designa.Numero
         And Scs_Actuaciondesigna.Idinstitucion_Proc = Scs_Procedimientos.Idinstitucion
  ...
  Begin
    ...
    v_Datoserror2 := 'Iniciando facturacion de Oficio';
    v_Codretorno2 := 0;

    v_Datoserror2 := 'Almacenando en los historicos';
    Pkg_Siga_Fcs_Historico.Proc_Fcs_Historicos_Turnos(p_Idinstitucion, p_Idfacturacion, v_Codretorno2, v_Datoserror2);

    v_Datoserror2 := 'Obteniendo parametro de tipo de facturacion de las actuaciones de Fin';
    Select F_SIGA_GETPARAMETRO('FCS', 'FORMA_FACTURAR_ACTUACIONES_DESIGNA_DEFIN', p_idinstitucion)
      Into v_tipo_facturacion_fines
      From dual;

    v_Datoserror2 := 'Obteniendo FECHADESDE y FECHAHASTA a partir de la facturacion';
    ...
    v_Datoserror2 := 'Iniciando el recorrido por Grupos de facturacion';
    v_Total       := 0;
    For v_Gruposfacturacion In c_Gruposfacturacion Loop

      v_Datoserror2 := 'Iniciando el recorrido por Turnos';
      For v_Turno In c_Turno(v_Gruposfacturacion.Idgrupofacturacion) Loop
        ...
        For v_Actdesigna In c_Actdesigna(v_Turno.Idturno, v_Fechadesde, v_Fechahasta,v_Gruposfacturacion.factconvenio) Loop
          ...
          If v_tipo_facturacion_fines = C_FACTURA_FINES_RESTOINICIO And v_Actdesigna.Idacreditacion = C_ACREDITACION_FIN Then
            ...
            Begin
              ...
              Begin
                ...
                Select Distinct
                       (Select Sum(Facact_Ini.Importefacturado)
            ...
            If b_variosPrecios Then
              -- Si hay varias actuaciones de inicio con varios precios
              If n_inicios_facturados = n_fines_facturados And n_inicios_enestafacturacion = n_fines_enestafacturacion Then
                -- Esta es una excepcion a la regla: en el caso de que las actuaciones de inicio facturadas
                -- se correspondan directamente con las de fin facturadas
                -- y aparte tb las actuaciones de inicio de esta facturacion se correspondan directamente con las de fin
                -- solo en este caso (que realmente no es excepcion sino que es algo logico) se factura de forma normal.
                v_Precio     := v_Actdesigna.Precio;
            ...
            Else -- Si solo hay un precio de inicio, entonces se puede facturar sin problemas
              If n_Importe_Inicio_Facturado Is Not Null Then
                -- Si ya actuacion de inicio fue facturada antes, entonces aplicamos la resta
                v_Precio     := v_Actdesigna.Precio - n_Importe_Inicio_Facturado;
                If (v_Precio < 0) Then
                  -- no se puede facturar un valor negativo
                  v_Precio := 0;
                End If;
                v_Porcentaje := 100;
              Else
                -- Si la actuacion de inicio esta en esta misma facturacion, se factura normalmente, por porcentaje
                v_Precio     := v_Actdesigna.Precio;
                v_Porcentaje := v_Actdesigna.Porcentaje;
              End If;
            End If;
          ...
          v_Datoserror2 := 'Acumulando el total';
          v_Total := v_Total + Round(v_Precio * v_Porcentaje / 100, 2);

          v_Datoserror2 := 'Insertando el registro facturado';
          ...
          Values
            (p_Idinstitucion,
             ...
             Round(v_Precio * v_Porcentaje / 100, 2));

          v_Datoserror2 := 'Actualizando la actuacion a Facturada';
          Update Scs_Actuaciondesigna
             Set Facturado = Pkg_Siga_Constantes.Db_True_n, Idfacturacion = p_Idfacturacion
...
*/
  ...
  Procedure PROC_EXPORTAR_TURNOS_3001(p_Idinstitucion  In Number,
    ...
    Cursor c_Export(v_Idioma Adm_Lenguajes.Idlenguaje%Type, v_Idinstitucion Fcs_Fact_Apunte.Idinstitucion%Type, v_Idfacturacion1 Fcs_Facturacionjg.Idfacturacion%Type, v_Idfacturacion2 Fcs_Facturacionjg.Idfacturacion%Type, v_Idpersona Fcs_Fact_Apunte.Idpersona%Type) Is
      Select To_Number(Nvl(Col.Ncomunitario, Col.Ncolegiado)) As Numcol,
             ...
             Fad.Importefacturado As Importe,
  ...
  /****************************************************************************************************************/
    ...
    PROCEDURE PROC_FCS_CARGA_CONFIG_GUARDIA (
        ...
        CURSOR C_IDHITO(V_IDINSTITUCION NUMBER, V_IDTURNO NUMBER, V_IDGUARDIA NUMBER, V_IDFACTURACION NUMBER, V_CONSEJO CEN_INSTITUCION.CEN_INST_IDINSTITUCION%Type) IS
            SELECT HFG.IDHITO,
            ...
            WHERE HFG.IDINSTITUCION = V_IDINSTITUCION
                ...
                AND (V_IDFACTURACION IS Null Or V_CONSEJO <> C_CATALAN)
            UNION
            SELECT HHTG.IDHITO,
            ...
            WHERE HHTG.IDINSTITUCION = V_IDINSTITUCION
                ...
                And V_CONSEJO = C_CATALAN -- Solo es valido para los colegios catalanes en este codigo (123_009)
        ORDER BY IDHITO;
    ...
    BEGIN
        -- Obteniendo si el colegio es catalan
        SELECT CEN_INST_IDINSTITUCION
          INTO V_CONSEJO
          FROM CEN_INSTITUCION
         WHERE IDINSTITUCION = P_IDINSTITUCION;
        ...
        -- Para cada Hito
        FOR V_IDHITO IN C_IDHITO(P_IDINSTITUCION, P_IDTURNO, P_IDGUARDIA, P_IDFACTURACION, V_CONSEJO) LOOP
  ...
  /***************************************************************************/
    ...
    CURSOR C_ASISTENCIASTIPO_UG (V_IDINSTITUCION NUMBER, V_IDTURNO NUMBER, V_IDGUARDIA NUMBER, V_IDPERSONA NUMBER, V_FECHAFIN DATE, V_IDTIPOASISTCOLEG NUMBER, V_DIASAPLICABLES VARCHAR2, V_IDFACTURACION NUMBER) IS (
        SELECT ASI.IDTIPOASISTENCIACOLEGIO,
           ...
           ASI.NUMERO,
           1 AS FACTURADO,
           (Select Sum(facasi.precioaplicado) From Fcs_Fact_Asistencia facasi Where facasi.Idinstitucion = asi.Idinstitucion And facasi.anio = asi.anio And facasi.numero = asi.numero) AS IMPORTE,
           FUNC_CALCULAR_IMPORTEASIST(ASI.IDINSTITUCION, ASI.IDTURNO, ASi.IDGUARDIA, ASI.IDTIPOASISTENCIACOLEGIO, V_FECHAFIN, V_IDFACTURACION, 1) AS IMPORTEMAX
        FROM SCS_ASISTENCIA ASI
        WHERE ASI.IDINSTITUCION = V_IDINSTITUCION
            AND ASI.IDTURNO = V_IDTURNO
            AND ASI.IDGUARDIA = V_IDGUARDIA
        ...
        SELECT ASI.IDTIPOASISTENCIACOLEGIO,
           ...
           ASI.NUMERO,
           0 AS FACTURADO,
           FUNC_CALCULAR_IMPORTEASIST(ASI.IDINSTITUCION, ASI.IDTURNO, ASI.IDGUARDIA, ASI.IDTIPOASISTENCIACOLEGIO, V_FECHAFIN, V_IDFACTURACION, 0) AS IMPORTE,
           FUNC_CALCULAR_IMPORTEASIST(ASI.IDINSTITUCION, ASI.IDTURNO, ASi.IDGUARDIA, ASI.IDTIPOASISTENCIACOLEGIO, V_FECHAFIN, V_IDFACTURACION, 1) AS IMPORTEMAX
        FROM SCS_ASISTENCIA ASI
        WHERE ASI.IDINSTITUCION = V_IDINSTITUCION
            AND ASI.IDTURNO = V_IDTURNO
  ...
  Begin
    If p_Idinstitucion = c_Id_Gijon Then
      Return f_Tiene_Asist_Derivadas_Gijon(p_Idinstitucion,
                                           ...
                                           p_Fechadesde,
                                           p_Fechahasta);
    Elsif p_Idinstitucion in (c_Id_Albacete, c_Id_Ciudad_Real, c_Id_La_Rioja, c_Id_Soria, c_Id_Valladolid, c_Id_Zamora) Then
        ...
        FOR V_ACTUACIONES_FG IN C_ACTUACIONESTIPO_FG(M_CG_FACTURABLE(indiceMatrizFacturable).IDINSTITUCION,
          ...
          if (V_ACTUACIONES_FG.facturado <> pkg_siga_constantes.DB_TRUE_A or
            ...
            costeFijoAc := costeFijoAc + V_ACTUACIONES_fG.costefijo;
              ...
              M_APUNTE_AC(IND_AC).CONTADOR := contadorUG;
              M_APUNTE_AC(IND_AC).costefijo := V_ACTUACIONES_fG.costefijo;
